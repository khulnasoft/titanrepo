...
const titanPath = path.join(__dirname, "../titan" + (isWin ? ".exe" : ""));
...
export class Monorepo {
  ...
  constructor(name: string) {
    this.root = fs.mkdtempSync(path.join(__dirname, `titan-monorepo-${name}-`));
  }

  init(npmClient: NPMClient, titanConfig = {}, subdir?: string) {
  ...
  `
    ...
    this.generateRepoFiles(titanConfig);
  }
  ...
  generateRepoFiles(titanConfig = {}) {
    this.commitFiles({
      ...
      "package.json": {
        ...
        scripts: {
          build: `echo building`,
          test: `${titanPath} run test`,
          lint: `${titanPath} run lint`,
...
`,
  ...
  titan(
    ...
    return execa.sync(titanPath, [command, ...resolvedArgs], {
      cwd: this.root,
      shell: true,
      ...options,
    });
