...
for (let npmClient of ["yarn", "berry", "pnpm6", "pnpm", "npm"] as const) {
  ...
  suites.push(Suite);
  // test that titan can run from a subdirectory
}
...
function runSmokeTests<T>(
  ...
  suite(
    ...
    async () => {
      const results = repo.titan("run", ["build", "--dry=json"], options);
      ...
      const secondRun = repo.titan("run", ["build", "--dry=json"], options);
      const secondDryRun: DryRun = JSON.parse(secondRun.stdout);

      repo.titan("run", ["build"], options);

      const thirdRun = repo.titan("run", ["build", "--dry=json"], options);
  ...
  suite(
    ...
    async () => {
      const results = repo.titan("run", ["lint"], options);
      assert.equal(0, results.exitCode, "exit code should be 0");
      const commandOutput = getCommandOutputAsArray(results);
  ...
  suite(
    ...
    async () => {
      ...
      const sinceCommandOutputNoCache = getCommandOutputAsArray(
        repo.titan("run", ["test", "--since=main", "--no-cache"], options)
      );
      ...
      const commandOnceBHasChangedOutput = getCommandOutputAsArray(
        repo.titan("run", ["test"], options)
      );

      assert.fixture(
        `• Packages in scope: a, b, c`,
        commandOnceBHasChangedOutput[0],
        "After running, changing source of b, and running `titan run test` again, should print `Packages in scope: a, b, c`"
      );
      assert.fixture(
        `• Running test in 3 packages`,
        commandOnceBHasChangedOutput[1],
        "After running, changing source of b, and running `titan run test` again, should print `Running in 3 packages`"
      );
      assert.ok(
        commandOnceBHasChangedOutput.findIndex((l) =>
          l.startsWith("a:test: cache miss, executing")
        ) >= 0,
        "After running, changing source of b, and running `titan run test` again, should print `a:test: cache miss, executing` since a depends on b and b has changed"
      );
      assert.ok(
        commandOnceBHasChangedOutput.findIndex((l) =>
          l.startsWith("b:test: cache miss, executing")
        ) >= 0,
        "After running, changing source of b, and running `titan run test` again, should print `b:test: cache miss, executing` since b has changed"
      );
      assert.ok(
        commandOnceBHasChangedOutput.findIndex((l) =>
          l.startsWith("c:test: cache hit, replaying output")
        ) >= 0,
        "After running, changing source of b, and running `titan run test` again, should print `c:test: cache hit, replaying output` since c should not be impacted by changes to b"
      );

      const scopeCommandOutput = getCommandOutputAsArray(
        repo.titan("run", ["test", '--scope="!b"'], options)
      );
  ...
  suite(
    ...
    async () => {
      const result = getCommandOutputAsArray(
        repo.titan("run", ["special"], options)
      );
  ...
  suite(
    ...
    async () => {
      const expectArgsPassed = (inputArgs: string[], passedArgs: string[]) => {
        const result = getCommandOutputAsArray(
          repo.titan("run", inputArgs, options)
        );
  ...
  if (["npm", "yarn", "pnpm6", "pnpm", "berry"].includes(npmClient)) {
    ...
    suite(
      ...
      async () => {
        ...
        }, `Could not read generated \`out\` directory after \`titan prune\``);
        const expected = [
          "out/package.json",
          "out/titan.json",
    ...
    suite(
      `${npmClient} + titan prune --docker${
        options.cwd ? " from " + options.cwd : ""
      }`,
      async () => {
        const pruneCommandOutput = getCommandOutputAsArray(
          repo.titan("prune", ["--scope=a", "--docker"], options)
        );
        assert.fixture(pruneCommandOutput[1], " - Added a");
        assert.fixture(pruneCommandOutput[2], " - Added b");
