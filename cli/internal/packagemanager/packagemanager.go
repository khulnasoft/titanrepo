...
func GetPackageManager(projectDirectory titanpath.AbsoluteSystemPath, pkg *fs.PackageJSON) (packageManager *PackageManager, err error) {
...
func detectPackageManager(projectDirectory titanpath.AbsoluteSystemPath) (packageManager *PackageManager, err error) {
	for _, packageManager := range packageManagers {
	...
	return nil, errors.New(util.Sprintf("We did not detect an in-use package manager for your project. Please set the \"packageManager\" property in your root package.json (${UNDERLINE}https://nodejs.org/api/packages.html#packagemanager)${RESET} or run `npx @titan/codemod add-package-manager` in the root of your monorepo."))
}
...
// CanPrune returns if titan can produce a pruned workspace. Can error if fs issues occur
func (pm PackageManager) CanPrune(projectDirectory titanpath.AbsoluteSystemPath) (bool, error) {
	if pm.canPrune != nil {
...
func (pm PackageManager) ReadLockfile(projectDirectory titanpath.AbsoluteSystemPath) (lockfile.Lockfile, error) {
...
func (pm PackageManager) PrunePatchedPackages(pkgJSON *fs.PackageJSON, patches []titanpath.AnchoredUnixPath) error {
