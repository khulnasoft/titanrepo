...
import (
	...
	"github.com/khulnasoft/titanrepo/cli/internal/analytics"
	"github.com/khulnasoft/titanrepo/cli/internal/cache"
	"github.com/khulnasoft/titanrepo/cli/internal/cmdutil"
	"github.com/khulnasoft/titanrepo/cli/internal/colorcache"
	"github.com/khulnasoft/titanrepo/cli/internal/context"
	"github.com/khulnasoft/titanrepo/cli/internal/core"
	"github.com/khulnasoft/titanrepo/cli/internal/daemon"
	"github.com/khulnasoft/titanrepo/cli/internal/daemonclient"
	"github.com/khulnasoft/titanrepo/cli/internal/fs"
	"github.com/khulnasoft/titanrepo/cli/internal/graphvisualizer"
	"github.com/khulnasoft/titanrepo/cli/internal/logstreamer"
	"github.com/khulnasoft/titanrepo/cli/internal/nodes"
	"github.com/khulnasoft/titanrepo/cli/internal/packagemanager"
	"github.com/khulnasoft/titanrepo/cli/internal/process"
	"github.com/khulnasoft/titanrepo/cli/internal/runcache"
	"github.com/khulnasoft/titanrepo/cli/internal/scm"
	"github.com/khulnasoft/titanrepo/cli/internal/scope"
	"github.com/khulnasoft/titanrepo/cli/internal/signals"
	"github.com/khulnasoft/titanrepo/cli/internal/spinner"
	"github.com/khulnasoft/titanrepo/cli/internal/taskhash"
...
func configureRun(base *cmdutil.CmdBase, opts *Opts, signalWatcher *signals.Watcher) *run {
	if os.Getenv("TITAN_FORCE") == "true" {
		opts.runcacheOpts.SkipReads = true
	}

	if os.Getenv("TITAN_REMOTE_ONLY") == "true" {
...
func (r *run) run(ctx gocontext.Context, targets []string) error {
	...
	r.opts.cacheOpts.RemoteCacheOpts = titanJSON.RemoteCacheOptions
...
var (
	_profileHelp = `File to write titan's performance profile output into.
...
func addRunOpts(opts *runOpts, flags *pflag.FlagSet, aliases map[string]string) {
	...
	flags.BoolVar(&opts.noDaemon, "no-daemon", false, "Run without using titan's daemon process")
	flags.BoolVar(&opts.singlePackage, "single-package", false, "Run titan in single-package mode")
	// This is a no-op flag, we don't need it anymore
	flags.Bool("experimental-use-daemon", false, "Use the experimental titan daemon")
...
func (r *run) executeTasks(ctx gocontext.Context, g *completeGraph, rs *runSpec, engine *core.Engine, packageManager *packagemanager.PackageManager, hashes *taskhash.Tracker, startAt time.Time) error {
	...
	if err != nil {
		if errors.Is(err, cache.ErrNoCachesEnabled) {
			r.base.LogWarning("No caches are enabled. You can try \"titan login\", \"titan link\", or ensuring you are not passing --remote-only to enable caching", nil)
		} else {
			return errors.Wrap(err, "failed to set up caching")
		}
	}
	defer func() {
		_ = spinner.WaitFor(ctx, titanCache.Shutdown, r.base.UI, "...writing to cache...", 1500*time.Millisecond)
	}()
	colorCache := colorcache.New()
	runState := NewRunState(startAt, rs.Opts.runOpts.profile)
	runCache := runcache.New(titanCache, r.base.RepoRoot, rs.Opts.runcacheOpts, colorCache)
...
func (r *run) executeDryRun(ctx gocontext.Context, engine *core.Engine, g *completeGraph, taskHashes *taskhash.Tracker, rs *runSpec) ([]hashedTask, error) {
	...
	errs := engine.Execute(g.getPackageTaskVisitor(ctx, func(ctx gocontext.Context, packageTask *nodes.PackageTask) error {
		...
		itemStatus, err := titanCache.Exists(hash)
		if err != nil {
			return err
		}
...
func validateTasks(pipeline fs.Pipeline, tasks []string) error {
	for _, task := range tasks {
		if !pipeline.HasTask(task) {
			return fmt.Errorf("task `%v` not found in titan `pipeline` in \"titan.json\". Are you sure you added it?", task)
		}
	}
	return nil
}
...
func (ec *execContext) exec(ctx gocontext.Context, packageTask *nodes.PackageTask, deps dag.Set) error {
	...
	// TODO(gsoltis): if/when we fix https://github.com/khulnasoft/titanrepo/issues/937
	...
	// takes a RelativeSystemPath. Resolve during migration from titanpath.AbsoluteSystemPath to
	// AbsoluteSystemPath
	cmd.Dir = ec.repoRoot.UntypedJoin(packageTask.Pkg.Dir.ToStringDuringMigration()).ToString()
	envs := fmt.Sprintf("TITAN_HASH=%v", hash)
