...
func (p *prune) prune(opts *opts) error {
	...
	workspaces := []titanpath.AnchoredSystemPath{}
	...
	if fs.FileExists("titan.json") {
		if err := fs.CopyFile(&fs.LstatCachedFile{Path: p.base.RepoRoot.UntypedJoin("titan.json")}, fullDir.UntypedJoin("titan.json").ToStringDuringMigration()); err != nil {
			return errors.Wrap(err, "failed to copy root titan.json")
		}
	}
	...
	if opts.docker {
		...
		if err := fs.CopyFile(
			...
			outDir.Join(titanpath.RelativeUnixPath("json/package.json").ToSystemPath()).ToString(),
		); err != nil {
			return errors.Wrap(err, "failed to copy root package.json")
		}
	}
