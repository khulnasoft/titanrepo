...
func Test_BerryPruneDescriptors(t *testing.T) {
	...
	prunedLockfile, err := lockfile.Subgraph(
		[]titanpath.AnchoredSystemPath{
			titanpath.AnchoredUnixPath("packages/a").ToSystemPath(),
			titanpath.AnchoredUnixPath("packages/c").ToSystemPath(),
		},
	...
	prunedLockfile, err = lockfile.Subgraph(
		[]titanpath.AnchoredSystemPath{
			titanpath.AnchoredUnixPath("packages/b").ToSystemPath(),
			titanpath.AnchoredUnixPath("packages/c").ToSystemPath(),
		},
		[]string{"lodash@npm:4.17.21"},
	)
