...
func (p *PnpmLockfile) Subgraph(workspacePackages []titanpath.AnchoredSystemPath, packages []string) (Lockfile, error) {
...
func pruneImporters(importers map[string]ProjectSnapshot, workspacePackages []titanpath.AnchoredSystemPath) (map[string]ProjectSnapshot, error) {
...
func (p *PnpmLockfile) Patches() []titanpath.AnchoredUnixPath {
	if len(p.PatchedDependencies) == 0 {
		return nil
	}
	patches := make([]titanpath.AnchoredUnixPath, len(p.PatchedDependencies))
	i := 0
	for _, patch := range p.PatchedDependencies {
		patches[i] = titanpath.AnchoredUnixPath(patch.Path)
		i++
	}
	return patches
}

func (p *PnpmLockfile) resolveSpecifier(workspacePath titanpath.AnchoredUnixPath, name string, specifier string) (string, bool, error) {
