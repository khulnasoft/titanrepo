...
func newTest(t *testing.T, redirectedURL string) *testResult {
	...
	userConfigPath := fs.AbsoluteSystemPathFromUpstream(t.TempDir()).UntypedJoin("titanrepo")
...
func Test_run(t *testing.T) {
	...
	expectedURL := "login-url/titanrepo/token?redirect_uri=http://127.0.0.1:9789"
...
func Test_sso(t *testing.T) {
	...
	expectedTokenName := fmt.Sprintf("Titan CLI on %v via SAML/OIDC Single Sign-On", host)
