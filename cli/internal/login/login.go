...
// NewLoginCommand returns the cobra subcommand for titan login
func NewLoginCommand(helper *cmdutil.Helper) *cobra.Command {
	var ssoTeam string
	cmd := &cobra.Command{
		Use:           "login",
		Short:         "Login to your Khulnasoft account",
		SilenceErrors: true,
		SilenceUsage:  true,
...
func (l *login) run(ctx context.Context) error {
	...
	loginURL := fmt.Sprintf("%v/titanrepo/token?redirect_uri=%v", loginURLBase, redirectURL)
	...
	oss, err := newOneShotServer(rootctx, func(w http.ResponseWriter, r *http.Request) {
		query = r.URL.Query()
		http.Redirect(w, r, loginURLBase+"/titanrepo/success", http.StatusFound)
	}, defaultPort)
	if err != nil {
	...
	l.base.UI.Info("")
	l.base.UI.Info(util.Sprintf("%s Titanrepo CLI authorized for %s${RESET}", ui.Rainbow(">>> Success!"), userResponse.User.Email))
	l.base.UI.Info("")
	l.base.UI.Info(util.Sprintf("${CYAN}To connect to your Remote Cache. Run the following in the${RESET}"))
	l.base.UI.Info(util.Sprintf("${CYAN}root of any titanrepo:${RESET}"))
	l.base.UI.Info("")
	l.base.UI.Info(util.Sprintf("  ${BOLD}npx titan link${RESET}"))
	l.base.UI.Info("")
	return nil
}
...
func (l *login) loginSSO(ctx context.Context, ssoTeam string) error {
	...
	// Stop the spinner before we return to ensure terminal is left in a good state
	s.Stop("")
	// open https://khulnasoft.com/api/auth/sso?teamId=<TEAM_ID>&mode=login
	...
	if verifiedUser.TeamID != "" {
		...
		l.base.UI.Info(util.Sprintf("  For more info, see ${UNDERLINE}https://titan.khulnasoft.com/docs/features/remote-caching${RESET}"))
		l.base.UI.Info("")
...
func (l *login) verifyCachingEnabled(teamID string) error {
	...
	case util.CachingStatusDisabled:
		...
		if team.IsOwner() {
			...
			if shouldEnable {
				url := fmt.Sprintf("https://khulnasoft.com/teams/%v/settings/billing", team.Slug)
...
func getTokenAndRedirect(params url.Values) (string, string) {
	locationStub := "https://khulnasoft.com/notifications/cli-login-"
