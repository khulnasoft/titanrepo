...
func (pt *PackageTask) RepoRelativeLogFile() string {
	return filepath.Join(pt.Pkg.Dir.ToStringDuringMigration(), ".titan", fmt.Sprintf("titan-%v.log", pt.Task))
}
...
func (pt *PackageTask) HashableOutputs() fs.TaskOutputs {
	inclusionOutputs := []string{fmt.Sprintf(".titan/titan-%v.log", pt.Task)}
