    ...
    pub async fn put(
        ...
        .map(|res| {
            res.map(|bytes| bytes.freeze())
                .map_err(titanrepo_api_client::Error::from)
        });
            ...
            .await
        {
            ...
            Err(titanrepo_api_client::Error::ReqwestError(e)) if e.is_timeout() => {
                Err(CacheError::TimeoutError(hash.to_string()))
            }
            Err(titanrepo_api_client::Error::ReqwestError(e)) if e.is_connect() => {
