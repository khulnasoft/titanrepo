...
use titanrepo_telemetry::events::task::{FileHashMethod, PackageTaskEventBuilder};

use crate::{hash_object::hash_objects, Error, Git, SCM};

pub type GitHashes = HashMap<RelativeUnixPathBuf, String>;

pub const INPUT_INCLUDE_DEFAULT_FILES: &str = "$TITAN_DEFAULT$";
...
impl SCM {
    ...
    pub fn get_package_file_hashes<S: AsRef<str>>(
        ...
        // If the inputs contain "$TITAN_DEFAULT$", we need to include the "default"
        // file hashes as well. NOTE: we intentionally don't remove
        // "$TITAN_DEFAULT$" from the inputs if it exists in the off chance that
        // the user has a file named "$TITAN_DEFAULT$" in their package (pls
        // no).
...
impl Git {
    fn get_package_file_hashes<S: AsRef<str>>(
        ...
        // no inputs, and no $TITAN_DEFAULT$
        if inputs.is_empty() {
            return self.get_package_file_hashes_from_index(titan_root, package_path);
        }

        // we have inputs, but no $TITAN_DEFAULT$
        ...
        // we have inputs, and $TITAN_DEFAULT$
        self.get_package_file_hashes_from_inputs_and_index(titan_root, package_path, inputs)
    }
    ...
    fn get_package_file_hashes_from_inputs_and_index<S: AsRef<str>>(
        ...
        let manual_includes_hashes =
            self.get_package_file_hashes_from_inputs(titan_root, package_path, &includes, true)?;

        // only run the excludes search if there are excludes
        let manual_excludes_hashes = if !excludes.is_empty() {
            self.get_package_file_hashes_from_inputs(titan_root, package_path, &excludes, false)?
        } else {
            GitHashes::new()
        };
...
mod tests {
    ...
    fn test_get_package_deps() -> Result<(), Error> {
        ...
        // create a root titan.json
        let root_titan_json_path = repo_root.join_component("titan.json");
        root_titan_json_path.create_with_contents("{}")?;
        ...
        // create a package titan.json
        let titan_json_path = my_pkg_dir.join_component("titan.json");
        titan_json_path.create_with_contents("{}")?;
        ...
        // create a file that matches the $TITAN_DEFAULT$ token to test the edge case
        let token_file_path = my_pkg_dir.join_components(&["$TITAN_DEFAULT$"]);
        ...
        let all_expected = to_hash_map(&[
            ...
            ("titan.json", "9e26dfeeb6e641a33dae4961196235bdb965b21b"),
            (
                "dir/nested-file",
        ...
        all_expected.insert(
            RelativeUnixPathBuf::new("$TITAN_DEFAULT$").unwrap(),
        ...
        let input_tests: &[(&[&str], &[&str])] = &[
            (
                &["uncommitted-file"],
                &["package.json", "titan.json", "uncommitted-file"],
            ),
            (
                &["**/*-file"],
                &[
            ...
            (
                &["$TITAN_DEFAULT$", "!committed-file", "dir/ignored-file"],
                &[
                    "uncommitted-file",
                    "package.json",
                    "titan.json",
            ...
            (
                &["!committed-file", "$TITAN_DEFAULT$", "dir/ignored-file"],
                &[
                    "uncommitted-file",
                    "package.json",
                    "titan.json",
