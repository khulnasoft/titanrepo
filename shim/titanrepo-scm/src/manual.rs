...
pub(crate) fn get_package_file_hashes_without_git<S: AsRef<str>>(
    ...
    let include_pattern = if includes.is_empty() {
        ...
        //   downstream.
        let titan_g = to_glob("package.json")?;
        let package_g = to_glob("titan.json")?;
        includes.push(titan_g);
...
mod tests {
    ...
    fn test_get_package_file_hashes_from_processing_gitignore() {
        ...
        let mut file_hash: Vec<(&str, &str, Option<&str>)> = vec![
            ("titan.json", "titan.json-file-contents", None),
            ...
            (
                "child-dir/libA/titan.json",
                "lib-titan.json-content",
        ...
        for (raw_unix_path, contents, expected_hash) in file_hash.iter() {
            ...
            let file_path = titan_root.join_unix_path(unix_path);
        ...
        let hashes =
            get_package_file_hashes_without_git::<&str>(&titan_root, &pkg_path, &[], false)
                .unwrap();
        ...
        for (raw_unix_path, contents, expected_hash) in file_hash.iter() {
            ...
            if let Some(hash) = expected_hash {
                ...
                if (unix_pkg_file_path.ends_with("file")
                    ...
                    || unix_pkg_file_path.ends_with("titan.json"))
