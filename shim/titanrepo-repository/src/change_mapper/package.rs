use thiserror::Error;
use titanpath::{AnchoredSystemPath, AnchoredSystemPathBuf};
...
/// Does *not* use the `globalDependencies` in titan.json.
...
mod tests {
    ...
    async fn test_different_package_detectors() -> Result<(), anyhow::Error> {
        ...
        let titan_package_detector =
            GlobalDepsPackageChangeMapper::new(&pkg_graph, std::iter::empty::<&str>())?;
        let change_mapper = ChangeMapper::new(&pkg_graph, vec![], titan_package_detector);
