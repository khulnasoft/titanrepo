        ...
        let globs = match self {
            PackageManager::Pnpm | PackageManager::Pnpm6 => {
                ...
                if workspaces.packages.is_empty() {
                    return Err(anyhow!("pnpm-workspace.yaml: no packages found. Titanrepo requires pnpm workspaces and thus packages to be defined in the root pnpm-workspace.yaml"));
                } else {
                    workspaces.packages
                }
            }
            PackageManager::Berry | PackageManager::Npm | PackageManager::Yarn => {
                ...
                if package_json.workspaces.is_empty() {
                    return Err(anyhow!("package.json: no packages found. Titanrepo requires packages to be defined in the root package.json"));
                } else {
