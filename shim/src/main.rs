...
static TITAN_JSON: &str = "titan.json";
...
/// # Arguments
///
/// * `args`: Arguments for titan
///
/// returns: Result<i32, Error>
///
fn run_current_titan(args: Vec<String>) -> Result<i32> {
    let mut args = args
        .into_iter()
        .map(|s| {
...
impl RepoState {
    ...
    pub fn infer(current_dir: &Path) -> Result<Self> {
        // First we look for a `titan.json`. This iterator returns the first ancestor that contains
        // a `titan.json` file.
        let root_path = current_dir
            .ancestors()
            .find(|p| fs::metadata(p.join(TITAN_JSON)).is_ok());

        // If that directory exists, then we figure out if there are workspaces defined in it
        // NOTE: This may change with multiple `titan.json` files
...
/// Checks if either we have an explicit run command, i.e. `titan run build`
/// or an implicit run, i.e. `titan build`, where the command after `titan` is
...
/// then finding local titan installation. If the current binary is the local
/// titan installation, then we run current titan. Otherwise we kick over to
/// the local titan installation.
///
/// # Arguments
///
...
/// returns: Result<i32, Error>
///
fn run_correct_titan(repo_root: &Path, args: Vec<String>) -> Result<i32> {
    let local_titan_path = repo_root.join("node_modules").join(".bin").join("titan");

    let current_titan_is_local_titan = local_titan_path == current_exe()?;
    // If the local titan path doesn't exist or if we are local titan, then we go ahead and run
    if !local_titan_path.try_exists()? || current_titan_is_local_titan {
        return run_current_titan(args);
    }
    ...
    let mut command = process::Command::new(local_titan_path)
        ...
        .spawn()
        .expect("Failed to execute titan.");
...
fn main() -> Result<()> {
    ...
    // Quick fix for --help.
    if clap_args.help {
        let exit_code = run_current_titan(args)?;
    ...
    let exit_code = match run_correct_titan(&repo_state.root, args) {
