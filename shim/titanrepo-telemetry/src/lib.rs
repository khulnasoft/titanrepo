...
use titanrepo_ui::{color, ColorConfig, BOLD, GREY};
use uuid::Uuid;
...
mod tests {
    ...
    use titanrepo_khulnasoft_api::telemetry::{TelemetryEvent, TelemetryGenericEvent};
    ...
    async fn test_batching() {
        ...
        let config =
            TelemetryConfig::new(temp_dir.join_components(&["titanrepo", "telemetry.json"]))
                .unwrap();
