...
impl crate::Lockfile for PnpmLockfile {
    ...
    fn titan_version(&self) -> Option<String> {
        let titan_version = self
            .importers
            .values()
            // Look through all of the workspace packages for a titan dependency
            // grab the first one we find.
            .find_map(|project| project.dependencies.titan_version())?;
        Some(titan_version.to_owned())
    }
}
...
impl DependencyInfo {
    ...
    fn titan_version(&self) -> Option<&str> {
        let (_specifier, version) = self.find_resolution("titan")?;
        Some(version)
    }
}
...
mod tests {
    ...
    const PNPM6_TITAN: &[u8] = include_bytes!("../../fixtures/pnpm6titan.yaml").as_slice();
    const PNPM8_TITAN: &[u8] = include_bytes!("../../fixtures/pnpm8titan.yaml").as_slice();
    ...
    #[test_case(
        ...
        "canary",
        Ok(Some(crate::Package {
            key: "titan@1.13.3-canary.1".into(),
...
";
    ...
    #[test_case(PNPM6_TITAN, Some("2.0.3") ; "v6")]
    #[test_case(PNPM8_TITAN, Some("2.0.3") ; "v8")]
    #[test_case(PNPM_V9, Some("1.13.3-canary.1") ; "v9")]
    fn test_titan_version(lockfile: &[u8], expected: Option<&str>) {
        let lockfile = PnpmLockfile::from_bytes(lockfile).unwrap();
        assert_eq!(lockfile.titan_version().as_deref(), expected);
    }
}
