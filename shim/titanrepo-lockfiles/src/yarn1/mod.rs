...
impl Lockfile for Yarn1Lockfile {
    ...
    fn titan_version(&self) -> Option<String> {
        // Yarn lockfiles can have multiple descriptors as a key e.g. titan@latest,
        // titan@1.2.3 We just check if the first descriptor is for titan and
        // return that. Using multiple versions of titan in a single project is
        // not supported.
        let key = self.inner.keys().find(|key| key.starts_with("titan@"))?;
        let entry = self.inner.get(key)?;
...
mod test {
    ...
    fn test_titan_version(lockfile: &str, expected: &str) {
        let lockfile = Yarn1Lockfile::from_str(lockfile).unwrap();
        assert_eq!(lockfile.titan_version().as_deref(), Some(expected));
    }
}
