...
impl Lockfile for BerryLockfile {
    ...
    fn titan_version(&self) -> Option<String> {
        let titan_ident = Ident::try_from("titan").expect("'titan' is valid identifier");
        let key = self
            .locator_package
            .keys()
            .find(|key| titan_ident == key.ident)?;
...
mod test {
    ...
    #[test]
    fn test_titan_version() {
        let data = LockfileData::from_bytes(include_bytes!("../../fixtures/berry.lock")).unwrap();
        let lockfile = BerryLockfile::new(data, None).unwrap();
        assert_eq!(lockfile.titan_version().as_deref(), Some("1.4.6"));
    }
}
