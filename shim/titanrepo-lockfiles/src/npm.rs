...
impl Lockfile for NpmLockfile {
    ...
    fn titan_version(&self) -> Option<String> {
        let titan_entry = self.packages.get("node_modules/titan")?;
        titan_entry.version.clone()
    }
}
...
mod test {
    ...
    #[test]
    fn test_titan_version() -> Result<(), Error> {
        let lockfile = NpmLockfile::load(include_bytes!("../fixtures/npm-lock.json"))?;
        assert_eq!(lockfile.titan_version().as_deref(), Some("1.5.5"));
        Ok(())
    }
}
