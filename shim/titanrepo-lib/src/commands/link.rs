...
use rand::Rng;
use thiserror::Error;
use titanrepo_api_client::{CacheClient, Client};
#[cfg(not(test))]
use titanrepo_ui::CYAN;
use titanrepo_ui::{DialoguerTheme, BOLD, GREY};
use titanrepo_khulnasoft_api::{CachingStatus, Space, Team};

use crate::{
    cli::LinkTarget,
    commands::CommandBase,
    config,
    gitignore::ensure_titan_is_gitignored,
...
pub async fn link(
    ...
    let token = base.config()?.token().ok_or_else(|| Error::TokenNotFound {
        command: base.color_config.apply(BOLD.apply_to("`npx titan login`")),
    })?;
    ...
    match target {
        LinkTarget::RemoteCache => {
            ...
            if modify_gitignore {
                ensure_titan_is_gitignored(&base.repo_root).map_err(|error| {
...
fn select_team<'a>(base: &CommandBase, teams: &'a [Team]) -> Result<SelectedTeam<'a>, Error> {
    ...
    let prompt = format!(
        ...
        base.color_config.apply(BOLD.apply_to(
            "Which Khulnasoft scope (and Remote Cache) do you want associated with this Titanrepo?",
        )),
        base.color_config
...
fn select_space<'a>(base: &CommandBase, spaces: &'a [Space]) -> Result<SelectedSpace<'a>, Error> {
    ...
    let prompt = format!(
        ...
        base.color_config.apply(
            BOLD.apply_to("Which Khulnasoft space do you want associated with this Titanrepo?",)
        ),
        base.color_config
            .apply(CYAN.apply_to("[Use arrows to move, type to filter]"))
    );
...
mod test {
    ...
    use crate::{
        ...
        config::TitanrepoConfigBuilder,
        titan_json::RawTitanJson,
        Args,
    };
    ...
    async fn test_link_remote_cache() -> Result<()> {
        ...
        assert!(
            team_id == Some(titanrepo_khulnasoft_api_mock::EXPECTED_USER_ID)
                || team_id == Some(titanrepo_khulnasoft_api_mock::EXPECTED_TEAM_ID)
        );
    ...
    async fn test_link_spaces() {
        ...
        let titan_json = RawTitanJson::parse(&titan_json_contents, "titan.json").unwrap();
        assert_eq!(
            titan_json.experimental_spaces.unwrap().id.unwrap(),
            titanrepo_khulnasoft_api_mock::EXPECTED_SPACE_ID.into()
        );
    }
}
