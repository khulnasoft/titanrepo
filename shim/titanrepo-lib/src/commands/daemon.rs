...
use titanrepo_ui::{color, BOLD_GREEN, BOLD_RED, GREY};
use which::which;
...
const DAEMON_NOT_RUNNING_MESSAGE: &str =
    "daemon is not running, run `titan daemon start` to start it";
...
async fn get_log_file_from_folder(base: &CommandBase) -> Result<String, DaemonError> {
    ...
    let log_folder = base.repo_root.join_components(&[".titan", "daemon"]);
...
pub async fn daemon_server(
    ...
    let server =
        crate::daemon::TitanGrpcService::new(base.repo_root.clone(), paths, timeout, exit_signal);
