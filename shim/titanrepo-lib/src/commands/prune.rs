...
pub enum Error {
    ...
    TitanJsonParser(#[from] crate::titan_json::parser::Error),
    #[error(transparent)]
...
impl<'a> Prune<'a> {
    ...
    fn copy_titan_json(&self, workspaces: &[String]) -> Result<(), Error> {
        let anchored_titan_path = titan_json();
        let original_titan_path = self.root.resolve(anchored_titan_path);
        let new_titan_path = self.full_directory.resolve(anchored_titan_path);

        let titan_json_contents = match original_titan_path.read_to_string() {
        ...
        let titan_json = RawTitanJson::parse(&titan_json_contents, anchored_titan_path.as_str())?;
