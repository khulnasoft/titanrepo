...
use thiserror::Error;
use tokio::select;
use titan_trace::TraceError;
use titanpath::AbsoluteSystemPathBuf;
use titanrepo_repository::{change_mapper::AllPackageChangeReason, package_graph::PackageName};
...
impl From<titanrepo_repository::change_mapper::PackageInclusionReason> for PackageChangeReason {
    fn from(value: titanrepo_repository::change_mapper::PackageInclusionReason) -> Self {
        match value {
            titanrepo_repository::change_mapper::PackageInclusionReason::All(
            ...
            titanrepo_repository::change_mapper::PackageInclusionReason::RootTask { task } => {
                PackageChangeReason::RootTask(RootTask {
                    task_name: task.to_string(),
                })
            }
            titanrepo_repository::change_mapper::PackageInclusionReason::ConservativeRootLockfileChanged => {
                PackageChangeReason::ConservativeRootLockfileChanged(ConservativeRootLockfileChanged { empty: false })
            }
            titanrepo_repository::change_mapper::PackageInclusionReason::LockfileChanged => {
                PackageChangeReason::LockfileChanged(LockfileChanged { empty: false })
            }
            titanrepo_repository::change_mapper::PackageInclusionReason::DependencyChanged {
                dependency,
            } => PackageChangeReason::DependencyChanged(DependencyChanged {
                dependency_name: dependency.to_string(),
            }),
            titanrepo_repository::change_mapper::PackageInclusionReason::DependentChanged {
                dependent,
            } => PackageChangeReason::DependentChanged(DependentChanged {
                dependent_name: dependent.to_string(),
            }),
            titanrepo_repository::change_mapper::PackageInclusionReason::FileChanged { file } => {
                PackageChangeReason::FileChanged(FileChanged {
                    file_path: file.to_string(),
                })
            }
            titanrepo_repository::change_mapper::PackageInclusionReason::InFilteredDirectory {
                directory,
            } => PackageChangeReason::InFilteredDirectory(InFilteredDirectory {
                directory_path: directory.to_string(),
            }),
            titanrepo_repository::change_mapper::PackageInclusionReason::IncludedByFilter {
                filters,
