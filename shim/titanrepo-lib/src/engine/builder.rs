...
use titanrepo_errors::{Spanned, TITAN_SITE};
use titanrepo_graph_utils as graph;
use titanrepo_repository::package_graph::{PackageGraph, PackageName, PackageNode, ROOT_PKG_NAME};
...
use crate::{
    ...
    titan_json::{
        validate_extends, validate_no_package_task_syntax, RawTaskDefinition, TitanJsonLoader,
    },
};
...
pub enum Error {
    ...
    #[diagnostic(
        code(missing_root_task_in_titan_json),
        url(
            "{}/messages/{}",
            TITAN_SITE, self.code().unwrap().to_string().to_case(Case::Kebab)
        )
    )]
    MissingRootTaskInTitanJson {
        task_id: String,
        #[label("add an entry in titan.json for this task")]
        span: Option<SourceSpan>,
        #[source_code]
        text: NamedSource,
    },
    ...
    #[error("Could not find \"{task_id}\" in root titan.json or \"{task_name}\" in package")]
    MissingPackageTask {
        #[label]
        span: Option<SourceSpan>,
        #[source_code]
...
pub struct EngineBuilder<'a> {
    ...
    titan_json_loader: Option<TitanJsonLoader>,
    is_single: bool,
...
impl<'a> EngineBuilder<'a> {
    pub fn new(
        repo_root: &'a AbsoluteSystemPath,
        package_graph: &'a PackageGraph,
        titan_json_loader: TitanJsonLoader,
        is_single: bool,
    ) -> Self {
        Self {
            repo_root,
            package_graph,
            titan_json_loader: Some(titan_json_loader),
            is_single,
            workspaces: Vec::new(),
            tasks: Vec::new(),
    ...
    pub fn build(mut self) -> Result<super::Engine, Error> {
        ...
        let mut titan_json_loader = self
            .titan_json_loader
            .take()
            .expect("engine builder cannot be constructed without TitanJsonLoader");
        ...
        let tasks: Vec<Spanned<TaskName<'static>>> = if self.add_all_tasks {
            ...
            if let Ok(titan_json) = titan_json_loader.load(&PackageName::Root) {
                tasks.extend(
                    titan_json
                        .tasks
                        .keys()
            ...
            for workspace in self.workspaces.iter() {
                let Ok(titan_json) = titan_json_loader.load(workspace) else {
                    continue;
                };
        ...
        for (workspace, task) in self.workspaces.iter().cartesian_product(tasks.iter()) {
            ...
            if Self::has_task_definition(&mut titan_json_loader, workspace, task, &task_id)? {
        ...
        if !missing_tasks.is_empty() {
            ...
            let errors = missing_tasks
                .into_iter()
                .map(|(name, span)| {
                    let (span, text) = span.span_and_text("titan.json");
        ...
        while let Some(task_id) = traversal_queue.pop_front() {
            ...
            {
                let (span, text) = task_id.span_and_text("titan.json");
                return Err(Error::MissingRootTaskInTitanJson {
                    span,
                    text,
    ...
    fn has_task_definition(
        loader: &mut TitanJsonLoader,
        workspace: &PackageName,
        task_name: &TaskName<'static>,
        task_id: &TaskId,
    ) -> Result<bool, Error> {
        let titan_json = loader.load(workspace).map_or_else(
            |err| {
                if matches!(err, config::Error::NoTitanJSON)
        ...
        if titan_json.tasks.contains_key(&task_id_as_name)
            || titan_json.tasks.contains_key(task_name)
        {
            Ok(true)
    ...
    fn task_definition(
        &self,
        titan_json_loader: &mut TitanJsonLoader,
    ...
    fn task_definition_chain(
        ...
        let root_titan_json = titan_json_loader.load(&PackageName::Root)?;
        ...
        if self.is_single {
            return match task_definitions.is_empty() {
                true => {
                    let (span, text) = task_id.span_and_text("titan.json");
                    Err(Error::MissingRootTaskInTitanJson {
                        span,
                        text,
                        task_id: task_id.to_string(),
                    })
        ...
        if task_id.package() != ROOT_PKG_NAME {
            match titan_json_loader.load(&PackageName::from(task_id.package())) {
                Ok(workspace_json) => {
...
fn validate_task_name(task: Spanned<&str>) -> Result<(), Error> {
    INVALID_TOKENS
        ...
        .map(|found_token| {
            let (span, text) = task.span_and_text("titan.json");
...
mod test {
    ...
    use crate::{
        engine::TaskNode,
        titan_json::{RawTitanJson, TitanJson},
    };
    ...
    impl Lockfile for MockLockfile {
        ...
        fn titan_version(&self) -> Option<String> {
            None
        }
    }
    ...
    fn titan_json(value: serde_json::Value) -> TitanJson {
        ...
        let raw = RawTitanJson::parse(&json_text, "").unwrap();
    ...
    fn test_task_definition(
        ...
        .into_iter()
        .collect();
        let mut loader = TitanJsonLoader::noop(titan_jsons);
    ...
    fn test_default_engine() {
        ...
        .into_iter()
        .collect();
        let loader = TitanJsonLoader::noop(titan_jsons);
    ...
    fn test_depend_on_missing_task() {
        ...
        assert_matches!(engine, Err(Error::MissingRootTaskInTitanJson { .. }));
    }
