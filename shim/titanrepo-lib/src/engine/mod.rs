...
use titanrepo_repository::package_graph::{PackageGraph, PackageName};

use crate::{run::task_id::TaskId, task_graph::TaskDefinition, titan_json::UIMode};
...
impl Engine<Built> {
    ...
    pub fn transitive_dependents(&self, task_id: &TaskId<'static>) -> HashSet<&TaskNode> {
        titanrepo_graph_utils::transitive_closure(
            &self.task_graph,
                    ...
                    .neighbors_directed(node_index, petgraph::Direction::Outgoing)
                {
                        ...
                        && package_json.scripts.contains_key(dep_id.task())
                    {
                        let (span, text) = self
                            .task_locations
                            .get(dep_id)
                            .map(|spanned| spanned.span_and_text("titan.json"))
...
pub enum ValidateError {
    ...
    #[error(
        "Cannot run interactive task \"{task}\" without experimental UI. Set `\"experimentalUI\": \
         true` in `titan.json` or `TITAN_EXPERIMENTAL_UI=true` as an environment variable"
    )]
    ...
    impl<'a> PackageDiscovery for DummyDiscovery<'a> {
        ...
        > {
            ...
            Ok(DiscoveryResponse {
                package_manager: titanrepo_repository::package_manager::PackageManager::Pnpm,
                workspaces,
            })
        }
