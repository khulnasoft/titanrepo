...
use titanrepo_ui::{ColorConfig, GREY};
...
impl From<OutputLogsMode> for titanrepo_ui::tui::event::OutputLogs {
    fn from(value: OutputLogsMode) -> Self {
        match value {
            OutputLogsMode::Full => titanrepo_ui::tui::event::OutputLogs::Full,
            OutputLogsMode::None => titanrepo_ui::tui::event::OutputLogs::None,
            OutputLogsMode::HashOnly => titanrepo_ui::tui::event::OutputLogs::HashOnly,
            OutputLogsMode::NewOnly => titanrepo_ui::tui::event::OutputLogs::NewOnly,
            OutputLogsMode::ErrorsOnly => titanrepo_ui::tui::event::OutputLogs::ErrorsOnly,
        }
    }
}
...
pub struct Args {
    ...
    ///
    /// `titan` will use hints from codebase to guess which package manager
    /// should be used.
    #[clap(long, global = true)]
    pub dangerously_disable_package_manager_check: bool,
    #[clap(long = "experimental-allow-no-titan-json", hide = true, global = true)]
    pub allow_no_titan_json: bool,
    /// Use the `titan.json` located at the provided path instead of one at the
    /// root of the repository.
    #[clap(long, global = true)]
    pub root_titan_json: Option<Utf8PathBuf>,
    ...
    pub fn track(&self, tel: &GenericEventBuilder) {
        ...
        if let Some(remote_cache_timeout) = self.remote_cache_timeout {
            tel.track_arg_value(
                ...
                titanrepo_telemetry::events::EventType::NonSensitive,
            );
        }
...
pub enum Command {
    ...
    /// Titan your monorepo by running a number of 'repo lints' to
...
pub struct RunArgs {
    ...
    // -----------------------
    /// Force titan to use the local daemon. If unset
    /// titan will use the default detection logic.
    #[clap(long, group = "daemon-group")]
    pub daemon: bool,

    /// Force titan to not use the local daemon. If unset
    /// titan will use the default detection logic.
    #[clap(long, group = "daemon-group")]
    pub no_daemon: bool,
...
pub async fn run(
    ...
    let should_print_version = env::var("TITAN_PRINT_VERSION_DISABLED")
        .map_or(true, |disable| !matches!(disable.as_str(), "1" | "true"))
        && !titanrepo_ci::is_ci();

    if should_print_version {
        eprintln!("{}\n", GREY.apply_to(format!("titan {}", get_version())));
    }
    ...
    // track system info
    root_telemetry.track_platform(TitanState::platform_name());
    root_telemetry.track_version(TitanState::version());
...
mod test {
    ...
    #[test_case::test_case(
        &["titan", "watch", "build", "--cache-dir", "foobar"],
        Args {
            command: Some(Command::Watch(Box::new(ExecutionArgs {
                tasks: vec!["build".to_string()],
                cache_dir: Some(Utf8PathBuf::from("foobar")),
    ...
    #[test_case::test_case(
        &["titan", "run", "build", "--daemon", "--no-daemon"],
    ...
    fn test_empty_cache_dir() {
        assert!(Args::try_parse_from(["titan", "build", "--cache-dir"]).is_err());
        assert!(Args::try_parse_from(["titan", "build", "--cache-dir="]).is_err());
        assert!(Args::try_parse_from(["titan", "build", "--cache-dir", ""]).is_err());
    }
    ...
    #[test]
    fn test_preflight() {
        assert!(!Args::try_parse_from(["titan", "build",]).unwrap().preflight);
        assert!(
            Args::try_parse_from(["titan", "build", "--preflight"])
                .unwrap()
                .preflight
        );
        assert!(Args::try_parse_from(["titan", "build", "--preflight=true"]).is_err());
    }
    ...
    fn test_dangerously_allow_no_package_manager() {
        assert!(
            !Args::try_parse_from(["titan", "build",])
                .unwrap()
    ...
    fn test_prevent_affected_and_filter() {
        assert!(
            Args::try_parse_from(["titan", "run", "build", "--affected", "--filter", "foo"])
                .is_err(),
        );
        assert!(Args::try_parse_from(["titan", "build", "--affected", "--filter", "foo"]).is_err(),);
        assert!(Args::try_parse_from(["titan", "build", "--filter", "foo", "--affected"]).is_err(),);
        assert!(Args::try_parse_from(["titan", "ls", "--filter", "foo", "--affected"]).is_err(),);
    }
}
