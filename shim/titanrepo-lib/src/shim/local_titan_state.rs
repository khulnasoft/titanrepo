...
use super::TitanState;

/// Structure that holds information on an existing local titan install
#[derive(Debug)]
...
pub fn titan_version_has_shim(version: &str) -> bool {
...
mod test {
    ...
    fn test_skip_infer_version_constraint(version: &str, expected: bool) {
        assert_eq!(titan_version_has_shim(version), expected);
    }
}
