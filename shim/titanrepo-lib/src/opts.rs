...
use titanrepo_api_client::APIAuth;
use titanrepo_cache::{CacheOpts, RemoteCacheOpts};
...
impl<'a> From<OptsInputs<'a>> for CacheOpts {
    fn from(inputs: OptsInputs<'a>) -> Self {
        let is_linked = titanrepo_api_client::is_linked(inputs.api_auth);
...
mod test {
    ...
    #[test_case    (
        ...
        "titan run build --filter=my-app --affected"
    )]
    #[test_case    (
        TestCaseOpts {
            tasks: vec!["build".to_string()],
