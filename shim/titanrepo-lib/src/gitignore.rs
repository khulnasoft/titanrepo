...
const TITAN_GITIGNORE_COMMENT: &str = "# Titanrepo";
const TITAN_GITIGNORE_ENTRY: &str = ".titan";
const GITIGNORE_FILE: &str = ".gitignore";

fn has_titan_gitignore_entry(mut lines: io::Lines<io::BufReader<File>>) -> bool {
    lines.any(|line| line.map_or(false, |line| line.trim() == TITAN_GITIGNORE_ENTRY))
}

fn get_ignore_string() -> String {
    format!("{}\n{}", TITAN_GITIGNORE_COMMENT, TITAN_GITIGNORE_ENTRY)
}

pub fn ensure_titan_is_gitignored(repo_root: &AbsoluteSystemPath) -> Result<(), io::Error> {
    ...
    if !gitignore_path.try_exists().unwrap_or(true) {
        ...
        let has_titan = has_titan_gitignore_entry(lines);
        if !has_titan {
...
mod tests {
    ...
    fn check_for_titan_in_gitignore_file(gitignore_path: &AbsoluteSystemPath) -> bool {
        let gitignore = gitignore_path
            .open()
            .expect("Failed to open .gitignore file");

        let lines = io::BufReader::new(gitignore).lines();
        has_titan_gitignore_entry(lines)
    }
    ...
    fn test_no_gitignore() -> Result<()> {
        ...
        ensure_titan_is_gitignored(repo_root).expect("Failed to ensure titan is gitignored");
        ...
        2. .titan
         */
        assert!(check_for_titan_in_gitignore_file(&gitignore_path));
        assert_eq!(get_gitignore_size(&gitignore_path), 2);
        Ok(())
    }

    #[test]
    fn gitignore_with_missing_titan() -> Result<()> {
        let repo_root_tmp = tempdir()?;
        let repo_root = AbsoluteSystemPath::from_std_path(repo_root_tmp.path())?;

        // create gitignore with no titan entry
        let gitignore_path = repo_root.join_component(GITIGNORE_FILE);
    ...
    #[test]
    fn gitignore_with_existing_titan_without_comment() -> Result<()> {
        let repo_root_tmp = tempdir()?;
        let repo_root = AbsoluteSystemPath::from_std_path(repo_root_tmp.path())?;
    ...
    #[test]
    fn gitignore_with_existing_titan_with_comment() -> Result<()> {
        let repo_root_tmp = tempdir()?;
        let repo_root = AbsoluteSystemPath::from_std_path(repo_root_tmp.path())?;
    ...
    #[test]
    fn gitignore_with_missing_titan_no_newline() -> Result<()> {
        let repo_root_tmp = tempdir()?;
        let repo_root = AbsoluteSystemPath::from_std_path(repo_root_tmp.path())?;
