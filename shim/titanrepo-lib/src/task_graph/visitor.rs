...
use titanrepo_ci::{Vendor, VendorBehavior};
...
enum TaskErrorCause {
    ...
    #[error("titan has internal error processing task")]
    Internal,
}
...
impl ExecContext {
    ...
    async fn execute_inner(
        ...
        match exit_status {
            ...
            // The child was killed by titan indicating a shutdown
...
impl<W: Write> TaskCacheOutput<W> {
    fn task_writer(&mut self) -> Either<titanrepo_ui::PrefixedWriter<&mut W>, TaskSender> {
        match self {
...
impl<W: Write> CacheOutput for TaskCacheOutput<W> {
    ...
    fn replay_logs(&mut self, log_file: &AbsoluteSystemPath) -> Result<(), titanrepo_ui::Error> {
        match self {
            TaskCacheOutput::Direct(direct) => {
                let writer = direct.output_prefixed_writer();
                titanrepo_ui::replay_logs(writer, log_file)
            }
            TaskCacheOutput::UI(task) => titanrepo_ui::replay_logs(task, log_file),
        }
    }
}
