...
use titanpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf, AnchoredSystemPathBuf, PathError};
...
use titanrepo_repository::package_manager;
...
impl FileWatching {
    /// This function is called in the constructor for the `TitanGrpcService`
    ...
    S: Future<Output = CloseReason>,
{
    ...
    pub async fn serve(self) -> Result<CloseReason, package_manager::Error> {
        ...
        let (service, exit_root_watch, watch_root_handle) =
            TitanGrpcServiceInner::new(repo_root.clone(), trigger_shutdown, paths.log_file);
...
#[tonic::async_trait]
impl proto::titand_server::Titand for TitanGrpcServiceInner {
    async fn hello(
        &self,
...
impl NamedService for TitanGrpcServiceInner {
...
mod test {
    ...
    use crate::daemon::{proto::VersionRange, CloseReason, Paths, TitanGrpcService};
