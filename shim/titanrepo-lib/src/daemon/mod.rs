//! The Titanrepo daemon watches files and pre-computes data to speed up titan's
...
pub use server::{CloseReason, TitanGrpcService};
...
fn daemon_log_file_and_folder(
    ...
    let log_folder = repo_root.join_components(&[".titan", "daemon"]);
    let log_file = log_folder.join_component(format!("{}-titan.log", repo_hash).as_str());
...
impl Paths {
    pub fn from_repo_root(repo_root: &AbsoluteSystemPath) -> Self {
        ...
        Self {
            pid_file: daemon_root.join_component("titand.pid"),
            lock_file: daemon_root.join_component("titand.lock"),
            sock_file: daemon_root.join_component("titand.sock"),
    ...
    impl From<titanrepo_repository::package_manager::PackageManager> for PackageManager {
        fn from(pm: titanrepo_repository::package_manager::PackageManager) -> Self {
            match pm {
                ...
                titanrepo_repository::package_manager::PackageManager::Pnpm9 => Self::Pnpm9,
...
mod test {
    ...
    fn test_repo_hash() {
        ...
        let (path, expected_hash) = ("/tmp/titanrepo", "6e0cfa616f75a61c");
        #[cfg(target_os = "windows")]
        let (path, expected_hash) = ("C:\\\\tmp\\titanrepo", "0103736e6883e35f");
