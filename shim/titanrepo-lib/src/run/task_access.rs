...
pub const TASK_ACCESS_CONFIG_PATH: [&str; 2] = [".titan", "traced-config.json"];
/// File name where the task is expected to leave a trace result
const TITAN_CONFIG_FILE: &str = "titan.json";
...
pub fn trace_file_path(
    ...
    repo_root.join_components(&[".titan", task_hash, TASK_ACCESS_TRACE_NAME])
}
...
fn task_access_trace_enabled(repo_root: &AbsoluteSystemPathBuf) -> Result<bool, std::io::Error> {
    ...
    let root_titan_json_path = &repo_root.join_component(TITAN_CONFIG_FILE);
    if root_titan_json_path.exists() {
        return Ok(false);
    }
...
impl TaskAccess {
    pub fn new(repo_root: AbsoluteSystemPathBuf, cache: AsyncCache, scm: &SCM) -> Self {
        ...
        if enabled {
            // make sure .titan is ignored
            match ensure_titan_is_gitignored(&repo_root) {
                Ok(_) => debug!("Automatically added .titan to .gitignore"),
                Err(e) => {
                    error!("Failed to add .titan to .gitignore. Caching will be disabled - {e}")
                }
            }
    ...
    async fn to_file(&self) -> Result<(), ToFileError> {
        ...
        if let Some(config_cache) = &self.config_cache {
            let traced_config =
                RawTitanJson::from_task_access_trace(&self.trace_by_task.lock().unwrap());
