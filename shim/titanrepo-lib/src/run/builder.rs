...
use titanrepo_cache::AsyncCache;
...
use titanrepo_telemetry::events::{
...
use crate::{
    ...
    titan_json::{TitanJson, TitanJsonLoader, UIMode},
    DaemonConnector,
};
...
impl RunBuilder {
    pub fn new(base: CommandBase) -> Result<Self, Error> {
        ...
        let root_titan_json_path = config.root_titan_json_path(&base.repo_root);
        let allow_no_titan_json = config.allow_no_titan_json();
    ...
    pub fn calculate_filtered_packages(
        ...
        if is_all_packages {
            for target in opts.run_opts.tasks.iter() {
                ...
                if root_titan_json.tasks.contains_key(&task_name) {
    ...
    pub async fn build(
        ...
        let is_linked = titanrepo_api_client::is_linked(&self.api_auth);
        ...
        run_telemetry.track_ci(titanrepo_ci::Vendor::get_name());
        ...
        let mut pkg_dep_graph = {
                ...
                && !self.allow_missing_package_manager
            {
                match (&daemon, self.opts.run_opts.daemon) {
                    (None, Some(true)) => {
                        ...
                        return Err(titanrepo_repository::package_graph::Error::Discovery(
            ...
            match graph {
                ...
                Err(titanrepo_repository::package_graph::Error::PackageJson(
                    package_json::Error::Io(io),
                )) if io.kind() == ErrorKind::NotFound => {
                    run_telemetry.track_error(TrackedErrors::InvalidPackageDiscovery);
                    return Err(titanrepo_repository::package_graph::Error::PackageJson(
                        package_json::Error::Io(io),
                    )
                    .into());
                }
        ...
        let mut titan_json_loader = if task_access.is_enabled() {
            TitanJsonLoader::task_access(
                self.repo_root.clone(),
                self.root_titan_json_path.clone(),
                root_package_json.clone(),
            )
        } else if is_single_package {
            TitanJsonLoader::single_package(
                self.repo_root.clone(),
                self.root_titan_json_path.clone(),
                root_package_json.clone(),
            )
        } else if self.allow_no_titan_json && !self.root_titan_json_path.exists() {
            TitanJsonLoader::workspace_no_titan_json(
                self.repo_root.clone(),
                pkg_dep_graph.packages(),
            )
        } else {
            TitanJsonLoader::workspace(
                self.repo_root.clone(),
                self.root_titan_json_path.clone(),
                pkg_dep_graph.packages(),
            )
        ...
        let root_titan_json = titan_json_loader.load(&PackageName::Root)?.clone();
    ...
    fn build_engine<'a>(
        ...
        .with_root_tasks(root_titan_json.tasks.keys().cloned())
