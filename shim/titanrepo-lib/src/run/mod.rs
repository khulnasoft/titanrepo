...
use titanrepo_repository::package_graph::{PackageGraph, PackageName, PackageNode};
use titanrepo_scm::SCM;
use titanrepo_telemetry::events::generic::GenericEventBuilder;
use titanrepo_ui::{
...
type UIResult<T> = Result<Option<(T, JoinHandle<Result<(), titanrepo_ui::Error>>)>, Error>;
...
impl Run {
    ...
    pub fn root_titan_json(&self) -> &TitanJson {
        &self.root_titan_json
    }
    ...
    pub async fn run(&self, ui_sender: Option<UISender>, is_watch: bool) -> Result<i32, Error> {
        ...
        let global_hash_inputs = {
            ...
            let pass_through_env = match env_mode {
                ...
                EnvMode::Strict => self.root_titan_json.global_pass_through_env.as_deref(),
            };
            ...
            get_global_hash_inputs(
                ...
                &self.root_titan_json.global_deps,
                &self.env_at_execution_start,
                &self.root_titan_json.global_env,
                pass_through_env,
                env_mode,
