...
use titanrepo_ui::sender::UISender;
...
pub struct WatchClient {
    ...
    ui_handle: Option<JoinHandle<Result<(), titanrepo_ui::Error>>>,
}
...
pub enum Error {
    ...
    UISend(String),
    #[error("cannot use root titan.json at {0} with watch mode")]
    NonStandardTitanJsonPath(String),
...
impl WatchClient {
    pub async fn new(base: CommandBase, telemetry: CommandEventBuilder) -> Result<Self, Error> {
        ...
        let root_titan_json_path = base.config()?.root_titan_json_path(&base.repo_root);
        if root_titan_json_path != base.repo_root.join_component(CONFIG_FILE) {
            return Err(Error::NonStandardTitanJsonPath(
                root_titan_json_path.to_string(),
            ));
        }
