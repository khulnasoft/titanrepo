...
use crate::{run::scope::change_detector::ScopeChangeDetector, titan_json::TitanJson};
...
impl<'a> FilterResolver<'a, ScopeChangeDetector<'a>> {
    pub(crate) fn new(
        ...
        inference: Option<PackageInference>,
        scm: &'a SCM,
        root_titan_json: &'a TitanJson,
    ) -> Result<Self, ResolutionError> {
        let global_deps = opts
            .global_deps
            .iter()
            .map(|s| s.as_str())
            .chain(root_titan_json.global_deps.iter().map(|s| s.as_str()));

        let change_detector =
            ScopeChangeDetector::new(titan_root, scm, pkg_graph, global_deps, vec![])?;
...
impl<'a, T: GitChangeDetector> FilterResolver<'a, T> {
    ...
    fn filter_nodes_with_selector(
        ...
        if let Some(globber) = parent_dir_globber.clone() {
            ...
            let path = self.titan_root.resolve(&base);
            if !path.exists() {
...
pub enum ResolutionError {
    ...
    GlobalDependenciesGlob(#[from] titanrepo_repository::change_mapper::Error),
}
