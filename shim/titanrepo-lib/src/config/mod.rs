...
use thiserror::Error;
use tracing::debug;
use titan_json::TitanJsonReader;
use titanpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf};
use titanrepo_errors::TITAN_SITE;
use titanrepo_repository::package_graph::PackageName;

pub use crate::titan_json::{RawTitanJson, UIMode};
use crate::{
    cli::{EnvMode, LogOrder},
    commands::CommandBase,
    titan_json::CONFIG_FILE,
};
...
#[diagnostic(
    ...
    url("{}/messages/{}", TITAN_SITE, self.code().unwrap().to_string().to_case(Case::Kebab))
)]
...
pub enum Error {
    ...
    #[error(
        "Could not find titan.json.\nFollow directions at https://titan.build/repo/docs to create \
         one"
    )]
    NoTitanJSON,
    #[error(transparent)]
    ...
    #[diagnostic(code(package_task_in_single_package_mode), url("{}/messages/{}", TITAN_SITE, self.code().unwrap().to_string().to_case(Case::Kebab)))]
    ...
    #[error("TITAN_REMOTE_CACHE_UPLOAD_TIMEOUT: error parsing timeout.")]
    InvalidUploadTimeout(#[source] std::num::ParseIntError),
    #[error("TITAN_PREFLIGHT should be either 1 or 0.")]
    InvalidPreflight,
    #[error("TITAN_LOG_ORDER should be one of: {0}")]
    InvalidLogOrder(String),
    #[error(transparent)]
    #[diagnostic(transparent)]
    TitanJsonParseError(#[from] crate::titan_json::parser::Error),
    #[error("found absolute path in `cacheDir`")]
    #[diagnostic(help("if absolute paths are required, use `--cache-dir` or `TITAN_CACHE_DIR`"))]
    AbsoluteCacheDir {
    ...
    #[error("Cannot load titan.json for in {0} single package mode")]
    InvalidTitanJsonLoad(PackageName),
}
...
#[setters(
    prefix = "with_",
    generate_delegates(ty = "TitanrepoConfigBuilder", field = "override_config")
)]
pub struct ConfigurationOptions {
    ...
    #[serde(skip)]
    pub(crate) root_titan_json_path: Option<AbsoluteSystemPathBuf>,
    ...
    pub(crate) allow_no_titan_json: Option<bool>,
}
...
impl ConfigurationOptions {
    ...
    pub fn root_titan_json_path(&self, repo_root: &AbsoluteSystemPath) -> AbsoluteSystemPathBuf {
        self.root_titan_json_path
            .clone()
            .unwrap_or_else(|| repo_root.join_component(CONFIG_FILE))
    }

    pub fn allow_no_titan_json(&self) -> bool {
        self.allow_no_titan_json.unwrap_or_default()
    }
}
...
impl TitanrepoConfigBuilder {
    ...
    pub fn build(&self) -> Result<ConfigurationOptions, Error> {
        ...
        let titan_json = TitanJsonReader::new(&self.repo_root);
...
mod test {
    ...
    use crate::config::{
        ConfigurationOptions, TitanrepoConfigBuilder, DEFAULT_API_URL, DEFAULT_LOGIN_URL,
        DEFAULT_TIMEOUT,
    };
    ...
    fn test_defaults() {
        ...
        .unwrap();
        assert_eq!(
            defaults.root_titan_json_path(repo_root),
            repo_root.join_component("titan.json")
        )
    }
    ...
    fn test_env_layering() {
        ...
        assert_eq!(config.team_id().unwrap(), khulnasoft_artifacts_owner);
        assert_eq!(config.token().unwrap(), khulnasoft_artifacts_token);
        assert_eq!(config.spaces_id().unwrap(), "my-spaces-id");
    }

    #[test]
    fn test_titan_json_remote_cache() {
        let tmp_dir = TempDir::new().unwrap();
        let repo_root = AbsoluteSystemPathBuf::try_from(tmp_dir.path()).unwrap();
        ...
        let team_id = "an-id";
        let titan_json_contents = serde_json::to_string_pretty(&serde_json::json!({
            "remoteCache": {
                "enabled": true,
                "apiUrl": api_url,
                "loginUrl": login_url,
        ...
        repo_root
            .join_component("titan.json")
            .create_with_contents(&titan_json_contents)
            .unwrap();
