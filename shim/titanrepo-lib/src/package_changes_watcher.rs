...
use titanpath::{AbsoluteSystemPathBuf, AnchoredSystemPath, AnchoredSystemPathBuf};
...
use titanrepo_scm::package_deps::GitHashes;

use crate::titan_json::{TitanJson, TitanJsonLoader, CONFIG_FILE};
...
struct RepoState {
    root_titan_json: Option<TitanJson>,
    ...
    fn get_change_mapper(&self) -> Option<ChangeMapper<GlobalDepsPackageChangeMapper>> {
        let Ok(package_change_mapper) = GlobalDepsPackageChangeMapper::new(
            ...
            self.root_titan_json
                .iter()
                .flat_map(|titan| titan.global_deps.iter())
                .map(|s| s.as_str()),
        ) else {
...
impl Subscriber {
    ...
    async fn initialize_repo_state(&self) -> Option<(RepoState, Gitignore)> {
        ...
        let root_titan_json = TitanJsonLoader::workspace(
                    ...
                    Ok(PackageChanges::Some(filtered_pkgs)) => {
                        ...
                        if filtered_pkgs.contains(&root_pkg) {
                            let has_root_tasks = repo_state
                                .root_titan_json
                                .as_ref()
                                .map_or(false, |titan| titan.has_root_tasks());
                            if !has_root_tasks {
