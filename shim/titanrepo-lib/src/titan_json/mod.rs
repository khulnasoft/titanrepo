...
use titanrepo_repository::package_graph::ROOT_PKG_NAME;
use titanrepo_unescape::UnescapedString;
...
pub use loader::TitanJsonLoader;
...
pub struct RawTaskDefinition {
    ...
    // instead of deriving them from a TitanJson
    #[serde(skip)]
...
pub const CONFIG_FILE: &str = "titan.json";
        ...
        for glob in outputs {
            if let Some(stripped_glob) = glob.value.strip_prefix('!') {
                if Utf8Path::new(stripped_glob).is_absolute() {
                    let (span, text) = glob.span_and_text("titan.json");
...
impl TryFrom<RawTaskDefinition> for TaskDefinition {
    ...
    fn try_from(raw_task: RawTaskDefinition) -> Result<Self, Error> {
        ...
        if let Some(interactive) = raw_task.interactive {
            let (span, text) = interactive.span_and_text("titan.json");
        ...
        if let Some(depends_on) = raw_task.depends_on {
            for dependency in depends_on.into_inner() {
                let (span, text) = dependency.span_and_text("titan.json");
        ...
        let inputs = raw_task
            ...
            .map(|input| {
                if Utf8Path::new(&input.value).is_absolute() {
                    let (span, text) = input.span_and_text("titan.json");
...
impl RawTitanJson {
    pub(crate) fn read(
        ...
        let raw_titan_json = RawTitanJson::parse(&contents, &root_relative_path)?;
...
impl TryFrom<RawTitanJson> for TitanJson {
    ...
    fn try_from(raw_titan: RawTitanJson) -> Result<Self, Error> {
        if let Some(pipeline) = raw_titan.pipeline {
            let (span, text) = pipeline.span_and_text("titan.json");
        ...
        for global_dep in raw_titan.global_dependencies.into_iter().flatten() {
            if global_dep.strip_prefix(ENV_PIPELINE_DELIMITER).is_some() {
                let (span, text) = global_dep.span_and_text("titan.json");
                return Err(Error::InvalidDependsOnValue {
        ...
        Ok(TitanJson {
            text: raw_titan.span.text,
            path: raw_titan.span.path,
            global_env: {
            ...
            tasks: raw_titan.tasks.unwrap_or_default(),
...
impl TitanJson {
    ...
    pub(crate) fn read(
        ...
        let raw_titan_json = RawTitanJson::read(repo_root, path)?;
...
pub fn validate_no_package_task_syntax(titan_json: &TitanJson) -> Vec<Error> {
    titan_json
        ...
        .map(|(task_name, entry)| {
            let (span, text) = entry.span_and_text("titan.json");
...
pub fn validate_extends(titan_json: &TitanJson) -> Vec<Error> {
    match titan_json.extends.first() {
        Some(package_name) if package_name != ROOT_PKG_NAME || titan_json.extends.len() > 1 => {
            let (span, text) = titan_json.extends.span_and_text("titan.json");
        ...
        None => {
            let path = titan_json
                .path
                .as_ref()
                .map_or("titan.json", |p| p.as_ref());

            let (span, text) = match titan_json.text {
                Some(ref text) => {
                    let len = text.len();
...
mod tests {
    ...
    use super::{RawTitanJson, Spanned, UIMode};
    use crate::{
        cli::OutputLogsMode,
        run::task_id::TaskName,
        task_graph::{TaskDefinition, TaskOutputs},
        titan_json::RawTaskDefinition,
    };
    ...
    fn test_titan_task_pruning() {
        let json = RawTitanJson::parse_from_serde(json!({
            "tasks": {
                "//#top": {},
                "build": {},
                "a#build": {},
        ...
        let expected: RawTitanJson = RawTitanJson::parse_from_serde(json!({
            "tasks": {
                "//#top": {},
                "build": {},
                "a#build": {},
    ...
    fn test_parsing_output_logs_mode(output_logs: &str, expected: Option<OutputLogsMode>) {
        let json: Result<RawTitanJson, _> = RawTitanJson::parse_from_serde(json!({
            "tasks": {
                "build": {
    ...
    fn test_ui(json: &str, expected: Option<UIMode>) {
        let json = RawTitanJson::parse(json, "").unwrap();
    ...
    fn test_daemon(json: &str, expected: &str) {
        let parsed = RawTitanJson::parse(json, "").unwrap();
    ...
    fn test_ui_serialization(input: &str, expected: &str) {
        let parsed = RawTitanJson::parse(input, "").unwrap();
        let actual = serde_json::to_string(&parsed).unwrap();
        assert_eq!(actual, expected);
    }
    ...
    fn test_allow_no_package_manager_serde(json_str: &str, expected: Option<bool>) {
        let json = RawTitanJson::parse(json_str, "").unwrap();
