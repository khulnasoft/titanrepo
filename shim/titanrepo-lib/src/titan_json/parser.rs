...
use thiserror::Error;
use titanrepo_errors::{ParseDiagnostic, WithMetadata};
use titanrepo_unescape::UnescapedString;

use crate::{
    run::task_id::TaskName,
    titan_json::{Pipeline, RawTaskDefinition, RawTitanJson, Spanned},
};

#[derive(Debug, Error, Diagnostic)]
#[error("failed to parse titan json")]
#[diagnostic(code(titan_json_parse_error))]
pub struct Error {
    #[related]
...
impl WithMetadata for RawTitanJson {
...
impl RawTitanJson {
    ...
    #[cfg(test)]
    pub fn parse_from_serde(value: serde_json::Value) -> Result<RawTitanJson, Error> {
        let json_string = serde_json::to_string(&value).expect("should be able to serialize");
        Self::parse(&json_string, "titan.json")
    }
    /// Parses a titan.json file into the raw representation with span info
    /// attached.
    ///
    /// # Arguments
    ///
    /// * `text`: The text contents of the titan.json file
    /// * `file_path`: The path to the titan.json file. Just used for error
    ///   display, so doesn't need to actually be a correct path.
    ///
    /// returns: Result<RawTitanJson, Error>
    pub fn parse(text: &str, file_path: &str) -> Result<RawTitanJson, Error> {
        let result = deserialize_from_json_str::<RawTitanJson>(
            text,
        ...
        let mut titan_json = result.into_deserialized().ok_or_else(|| Error {
            diagnostics: vec![],
            backtrace: backtrace::Backtrace::capture(),
        })?;

        titan_json.add_text(Arc::from(text));
        titan_json.add_path(Arc::from(file_path));
