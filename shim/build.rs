...
fn main() {
    ...
    let lib_search_path = if is_release {
        ...
        build_debug_libtitan()
    };
    println!("cargo:rustc-link-search={}", lib_search_path);
    println!("cargo:rustc-link-lib=titan");
...
fn expect_release_lib() -> String {
    ...
    let mut dir = PathBuf::from("libtitan");
    // format is ${BUILD_ID}_${OS}_${ARCH}. Build id is, for goreleaser reasons, titan-${OS}
    dir.push(format!("titan-{platform}_{platform}_{arch}"));
    dir.push("lib");
    dir.to_string_lossy().to_string()
}

fn build_debug_libtitan() -> String {
    let cli_path = "../cli";
    let mut cmd = new_command("make");
    cmd.current_dir(cli_path);
    cmd.arg("libtitan.a");
    let mut child = cmd.spawn().expect("failed to spawn make libtitan.a");
    child.wait().expect("failed to build libtitan.a");
    cli_path.to_string()
}
