...
#![deny(clippy::all)]
//! Titanrepo's library for authenticating with the Khulnasoft API.
...
use titanrepo_khulnasoft_api::{token::ResponseTokenMetadata, User};
...
pub const TITAN_TOKEN_DIR: &str = "titanrepo";
pub const TITAN_TOKEN_FILE: &str = "config.json";
    ...
    pub async fn has_cache_access<'a, T: CacheClient>(
            ...
            .await
        {
            ...
            Err(e) => match e {
                ...
                titanrepo_api_client::Error::UnknownStatus { code, .. } if code == "forbidden" => {
                    Ok(false)
                }
                // If the entire request fails with 403 also return false
                titanrepo_api_client::Error::ReqwestError(e)
...
mod tests {
    ...
    use titanrepo_khulnasoft_api::{token::Scope, CachingStatus, CachingStatusResponse};
