...
use titanrepo_ui::{start_spinner, ColorConfig, BOLD};
...
pub async fn login<T: Client + TokenClient + CacheClient>(
    ...
    if !force {
        if let Some(token) = existing_token {
            debug!("found existing titan token");
...
mod tests {
    ...
    impl LoginServer for MockLoginServer {
        async fn run(
            ...
            login_token
                .set(titanrepo_khulnasoft_api_mock::EXPECTED_TOKEN.to_string())
                .unwrap();
            Ok(())
        }
    }
    ...
    impl Client for MockApiClient {
        ...
        fn make_url(&self, endpoint: &str) -> titanrepo_api_client::Result<Url> {
            let url = format!("{}{}", self.base_url, endpoint);
            Url::parse(&url).map_err(|err| titanrepo_api_client::Error::InvalidUrl { url, err })
        }
    }
    ...
    impl TokenClient for MockApiClient {
        async fn get_metadata(
            &self,
            token: &str,
        ) -> titanrepo_api_client::Result<titanrepo_khulnasoft_api::token::ResponseTokenMetadata>
        {
            if token.is_empty() {
                return Err(MockApiError::EmptyToken.into());
            }

            Ok(titanrepo_khulnasoft_api::token::ResponseTokenMetadata {
                id: "id".to_string(),
                name: "name".to_string(),
                token_type: "token".to_string(),
                origin: "github".to_string(),
                scopes: vec![titanrepo_khulnasoft_api::token::Scope {
        ...
        async fn delete_token(&self, _token: &str) -> titanrepo_api_client::Result<()> {
            Ok(())
        }
    }
    ...
    impl CacheClient for MockApiClient {
        async fn get_artifact(
            ...
            _method: Method,
        ) -> Result<Option<Response>, titanrepo_api_client::Error> {
            unimplemented!("get_artifact")
        }
        async fn put_artifact(
            &self,
            _hash: &str,
            _artifact_body: impl titanrepo_api_client::Stream<
                    Item = Result<titanrepo_api_client::Bytes, titanrepo_api_client::Error>,
                > + Send
                + Sync
                + 'static,
            _body_len: usize,
        ...
        async fn get_caching_status(
            ...
            _team_id: Option<&str>,
            _team_slug: Option<&str>,
        ) -> Result<CachingStatusResponse, titanrepo_api_client::Error> {
    ...
    async fn test_login() {
        ...
        assert_eq!(&got_token, titanrepo_khulnasoft_api_mock::EXPECTED_TOKEN);
