...
use titanrepo_api_client::TokenClient;
use titanrepo_dirs::{config_dir, khulnasoft_config_dir};
use titanrepo_ui::{cprintln, GREY};

use crate::{
    Error, LogoutOptions, Token, TITAN_TOKEN_DIR, TITAN_TOKEN_FILE, KHULNASOFT_TOKEN_DIR,
    KHULNASOFT_TOKEN_FILE,
};
...
impl<T: TokenClient> LogoutOptions<T> {
    ...
    async fn remove_tokens(&self) -> Result<(), Error> {
        ...
        if let Some(titan_config_dir) = config_dir()? {
            self.try_remove_token(
                &titan_config_dir.join_components(&[TITAN_TOKEN_DIR, TITAN_TOKEN_FILE]),
            )
            .await?;
        }
...
mod tests {
    ...
    impl Client for MockApiClient {
        async fn get_user(&self, _token: &str) -> titanrepo_api_client::Result<UserResponse> {
            unimplemented!("get_user")
        }
        async fn get_teams(&self, _token: &str) -> titanrepo_api_client::Result<TeamsResponse> {
        ...
        fn make_url(&self, _endpoint: &str) -> titanrepo_api_client::Result<Url> {
    ...
    impl TokenClient for MockApiClient {
        async fn delete_token(&self, _token: &str) -> titanrepo_api_client::Result<()> {
            if self.succeed_delete_request {
                ...
                Err(titanrepo_api_client::Error::UnknownStatus {
                    code: "code".to_string(),
        ...
        async fn get_metadata(
            &self,
            _token: &str,
        ) -> titanrepo_api_client::Result<ResponseTokenMetadata> {
