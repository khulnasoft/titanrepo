...
//! as well as the architecture documentation in `packages/titan-vsc`.
...
pub struct Backend {
    ...
    // this is only used for titan optimize
...
impl LanguageServer for Backend {
            ...
            let daemon = match daemon {
                ...
                Err(DaemonConnectorError::Handshake(box DaemonError::VersionMismatch(message))) => {
                    self.client
                        .show_message(
                            MessageType::ERROR,
                            "Pre-2.0 versions of titanrepo are not compatible with 2.0 or later \
                             of the extension. If you do not plan to update to titan 2.0, please \
            ...
            capabilities: ServerCapabilities {
                ...
                execute_command_provider: Some(ExecuteCommandOptions {
                    commands: vec![
                        ...
                        // "titan.daemon.start".to_string(),
                        // "titan.daemon.status".to_string(),
                        // "titan.daemon.stop".to_string(),
                    ],
                    work_done_progress_options: Default::default(),
                }),
    ...
    /// Add code lens items for running a particular task in the titan.json
            ...
            match &diag.code {
                Some(NumberOrString::String(s)) if s == "deprecated:env-var" => {
                    code_actions.push(CodeActionOrCommand::CodeAction(CodeAction {
                        ...
                        command: Some(Command {
                            title: "Apply codemod".to_string(),
                            command: "titan.codemod".to_string(),
        ...
        let workspaces = packages.map(|p| {
            chain(
                ...
                iter::once(WorkspaceData {
                    ...
                    titan_json: root_titan_json.exists().then_some(root_titan_json),
                }),
            )
        });
                                ...
                                if string.value == property.name.as_str() {
                                    diagnostics.push(Diagnostic {
                                        ...
                                        code: Some(NumberOrString::String(
                                            "titan:self-dependency".to_string(),
                                        )),
                                        ..Default::default()
                                    });
                                    continue;
...
fn report_invalid_packages_and_tasks(
    ...
    let (package, task) = package_task
        .value
        .split_once('#') // titan packages may not have # in them
        .map(|(p, t)| (Some(p), t))
    ...
    match (tasks.get(task), package) {
        ...
        (_, Some(package)) if !packages.contains(&package) => {
            diagnostics.push(Diagnostic {
                ...
                range,
                severity: Some(DiagnosticSeverity::ERROR),
                code: Some(NumberOrString::String("titan:no-such-package".to_string())),
                ..Default::default()
            });
        }
                ...
                .contains(&package) =>
        {
            diagnostics.push(Diagnostic {
                ...
                severity: Some(DiagnosticSeverity::ERROR),
                code: Some(NumberOrString::String(
                    "titan:no-such-task-in-package".to_string(),
                )),
                ..Default::default()
            });
        }
        ...
        (None, None) => {
            diagnostics.push(Diagnostic {
                ...
                range,
                severity: Some(DiagnosticSeverity::ERROR),
                code: Some(NumberOrString::String("titan:no-such-task".to_string())),
                ..Default::default()
            });
        }
