...
import frameworks from '@titan/types/src/json/frameworks.json';

Environment variable inputs are a vital part of your applications that you'll need to account for in your Titanrepo configuration.

There are three important questions when working with environment variables in Titanrepo:

- [Are my environment variables accounted for in the task hash?](#adding-environment-variables-to-task-hashes)
- [Which Environment Mode will `titan` use?](#environment-modes)
...
<Callout type="good-to-know">
  Titanrepo also uses [System Environment
...
Titanrepo needs to be aware of your environment variables to account for changes in application behavior. To do this, use the `env` and `globalEnv` keys in your `titan.json` file.
...
<Callout type="good-to-know">
  Titanrepo supports wildcards for environment variables so you can easily
...
Titanrepo automatically adds prefix wildcards to your [`env`](/repo/docs/reference/configuration#env) key for common frameworks. If you're using one of the frameworks below in a package, you don't need to specify environment variables with these prefixes:
...
- [Strict Mode](#strict-mode) (Default): Filter environment variables to **only** those that are specified in the `env` and `globalEnv` keys in `titan.json`.
- [Loose Mode](#loose-mode): Allow all environment variables for the process to be available.

### Strict Mode

Strict Mode filters the environment variables available to a task's runtime to **only** those that are specified in the `globalEnv` and `env` keys in `titan.json`.
...
If any of these variables are important to your tasks and aren't included by [Framework Inference](#framework-inference), make sure they are in your `titan.json` configuration.
...
As long as the environment variable is available when `titan` is ran, your script will be able to use it. However, this also **lets you accidentally forget to account for an environment variable in your configuration much more easily**, allowing the task to hit cache when it shouldn't.
...
You then build your application using a value for `MY_API_URL` that targets your preview environment. When you're ready to ship your application, you build for production and see a cache hit - even though the value of the `MY_API_URL` variable has changed! `MY_API_URL` changed - but Titanrepo restored a version of your application from cache that uses the preview environment's `MY_API_URL` rather than production's.
...
`.env` files are great for working on an application locally. **Titanrepo does not load .env files into your task's runtime**, leaving them to be handled by your framework, or tools like [`dotenv`](https://www.npmjs.com/package/dotenv).

However, it's important that `titan` knows about changes to values in your `.env` files so that it can use them for hashing. If you change a variable in your `.env` files between builds, the `build` task should miss cache.
...
```json title="./titan.json"
{
  ...
  "tasks": {
    "build": {
      "inputs": ["$TITAN_DEFAULT$", ".env", ".env.local"] // Only the `build` task hash
    }
  }
}
```
...
### Use `eslint-config-titan`

[The `eslint-config-titan` package](/repo/docs/reference/eslint-config-titan) helps you find environment variables that are used in your code that aren't listed in your `titan.json`. This helps ensure that all your environment variables are accounted for in your configuration.

### Avoid creating or mutating environment variables at runtime

Titanrepo hashes the environment variables for your task at the beginning of the task. If you create or mutate environment variables during the task, Titanrepo will not know about these changes and will not account for them in the task hash.

For instance, Titanrepo will not be able to detect the inline variable in the example below:
...
`MY_VARIABLE` is being added to the environment _after_ the `dev` task has started, so `titan` will not be able to use it for hashing.
...
The `titan.json` below expresses:
...
[The `--summarize` flag](/repo/docs/reference/run#--summarize) can be added to your `titan run` command to produce a JSON file summarizing data about your task. Checking the diff for the `globalEnv` and `env` key can help you identify any environment variables that may be missing from your configuration.

## Next steps

Once you've accounted for your environment variables, you're ready to start building the CI pipelines that build, check, and deploy your applications, at the speed of `titan`.
