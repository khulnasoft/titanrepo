...
`titan` is built on top of [Workspaces](https://khulnasoft.com/docs/vercel-platform/glossary#workspace), a feature of package managers in the JavaScript ecosystem that allows you to group multiple packages in one repository.

Following these conventions is important because it allows you to:

- Lean on those conventions for all your repo's tooling
- Quickly, incrementally adopt Titanrepo into an existing repository

In this guide, we'll walk through setting up a multi-package workspace (monorepo) so we can set the groundwork for `titan`.

## Getting started

Setting up a workspace's structure can be tedious to do by hand. If you're new to monorepos, we recommend [using `create-titan` to get started](/repo/docs/getting-started/installation) with a valid workspace structure right away.
...
In JavaScript, a workspace can either be [a single package](/repo/docs/guides/single-package-workspaces) or a collection of packages. In these guides, we'll be focusing on [a multi-package workspace](https://khulnasoft.com/docs/vercel-platform/glossary#monorepo), often called a "monorepo".

Below, the structural elements of `create-titan` that make it a valid workspace are highlighted.
...
- [Root `titan.json`](#root-titanjson)
...
<Callout type="error">
Titanrepo does not support nested packages like `apps/**` or `packages/**`. Using a structure that would put a package at `apps/a` and another at `apps/a/b` will result in an error.
...
In the directory of the package, there must be a `package.json` to make the package discoverable to your package manager and `titan`. The [requirements for the `package.json` of a package](#anatomy-of-a-package) are below.
...
`titan.json` is used to configure the behavior of `titan`. To learn more about how to configure your tasks, visit the [Configuring tasks](/repo/docs/crafting-your-repository/configuring-tasks) page.

### Package manager lockfile

A lockfile is key to reproducible behavior for both your package manager and `titan`. Additionally, Titanrepo uses the lockfile to understand the dependencies between your [Internal Packages](/repo/docs/core-concepts/internal-packages) within your Workspace.

<Callout type="warn">
  If you do not have a lockfile present when you run `titan`, you may see
...
The `scripts` field is used to define scripts that can be run in the package's context. Titanrepo will use the name of these scripts to identify what scripts to run (if any) in a package. We talk more about these scripts on the [Running Tasks](/repo/docs/crafting-your-repository/running-tasks) page.
