---
title: run
description: API reference for the `titan run` command
---

import { Callout } from '#/components/callout';

Run tasks specified in `titan.json`.

```bash title="Terminal"
titan run [tasks] [options] [-- [args passed to tasks]]
```

- **[tasks]**: Titanrepo can run one or many tasks at the same time. To run a task through `titan`, it must be specified in `titan.json`.
- **[options]**: Options are used to control the behavior of the `titan run` command. Available flag options are described below.
...
<Callout type="good-to-know">
  `titan run` is aliased to `titan`. `titan run build lint check-types` is the
  same as `titan build lint check-types`. We recommend [using `titan run` in CI
  pipelines](/repo/docs/crafting-your-repository/constructing-ci#use-titan-run-in-ci)
  and `titan` with [global `titan`
  locally](/repo/docs/getting-started/installation#global-installation) for ease
  of use.
</Callout>

If no tasks are provided, `titan` will display what tasks are available for the packages in the repository.
...
TITAN_SCM_BASE=development titan run build --affected

# Override Git comparison head
TITAN_SCM_HEAD=your-branch titan run build --affected
```
...
```bash title="Terminal"
titan run build --cache-dir="./my-cache"
```
...
The same behavior can also be set via the `TITAN_CACHE_DIR=example/path` system variable.
...
```bash title="Terminal"
titan run build --concurrency=50%
titan run test --concurrency=5
```
...
When `--continue` is `true`, `titan` will exit with the highest exit code value encountered during execution.
...
Default: Directory of root `titan.json`

Set the working directory of the command.

```bash title="Terminal"
titan run build --cwd=./somewhere/else/in/your/repo
```
...
<Callout type="info">
  ...
  `titan.json`](/repo/docs/reference/configuration#dangerouslydisablepacakgemanagercheck)
  or the
  [`TITAN_DANGEROUSLY_DISABLE_PACKAGE_MANAGER_CHECK`](/repo/docs/reference/system-environment-variables)
...
The same behavior can also be set via the `TITAN_ENV_MODE=strict` system variable.
...
| ----------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| Package     | Select a package by its name in `package.json`.                                                                            | `titan run build --filter=ui`       |
| Directory   | Specify directories to capture a list of packages to run tasks. **When used with other filters, must be wrapped in `{}`**. | `titan run build --filter=./apps/*` |
| Git commits | Using Git specifiers, specify packages with source control changes. **Must be wrapped in `[]`**.                           | `titan run build --filter=[HEAD^1]` |
...
The same behavior can also be set via [the `TITAN_FORCE` environment variable](/repo/docs/reference/system-environment-variables).
...
```bash title="Terminal"
titan run build --framework-inference=false
```
...
titan run build --global-deps=".env.*" --global-deps=".eslintrc" --global-deps="jest.config.js"
```
...
<Callout type="info">
  ...
  in `titan.json` to make sure they are always accounted for.
</Callout>
...
By default, `titan` will use `grouped` logs in CI environments and `stream` logs everywhere else. This flag is not applicable when using [the terminal UI](https://titan.khulnasoft.com/repo/docs/reference/configuration#ui).
...
| `auto`    | Titan decides based on its own heuristics |
...
```bash title="Terminal"
titan run dev --no-cache
```

### `--daemon` and `--no-daemon`

`titan` can run a background process to pre-calculate values used for determining work that needs to be done. This standalone process (daemon) is an optimization, and not required for proper functioning of `titan`.

Passing `--daemon` instructs `titan` to use the standalone process, while `--no-daemon` instructs `titan` to avoid using or creating the standalone process.

The same behavior can also be set via the `TITAN_DAEMON=true` system variable.

### `--output-logs <option>`

Default: `full`

Set type of output logging, overriding [`outputLogs`](/repo/docs/reference/configuration#outputlogs) if it's defined in `titan.json`.

```bash title="Terminal"
titan run build --output-logs=errors-only
```
...
Additionally, `--only` will only run tasks in specified packages, excluding dependencies. For example, `titan run build --filter=web --only`, will **only** run the `build` script in the `web` package.
...
```bash title="Terminal"
titan run lint --parallel
titan run dev --parallel
```

<Callout type="info">
  The `--parallel` flag is typically used for long-running "dev" or "watch"
  tasks that don't exit. Starting in `titan@1.7`, we recommend configuring these
...
```bash title="Terminal"
titan run build --preflight
```

The same behavior can also be set via the `TITAN_PREFLIGHT=true` system variable.
...
```bash title="Terminal"
titan run build --profile=profile.json -vvv
```
...
```bash title="Terminal"
titan run build --remote-cache-timeout=60
```
...
Generates a JSON file in `.titan/runs` containing metadata about the run, including:
...
```bash title="Terminal"
titan run build --summarize
```

This flag can be helpful for debugging to determine things like:

- How `titan` interpreted your glob syntax for `inputs` and `outputs`
...
```bash title="Terminal"
titan run build --team=my-team --token=xxxxxxxxxxxxxxxxx
```

This value can also be set using [the `TITAN_TOKEN` system variable](/repo/docs/reference/system-environment-variables). If both are present, the flag value will override the system variable.
...
```bash title="Terminal"
titan run build --team=my-team
titan run build --team=my-team --token=xxxxxxxxxxxxxxxxx
```

This value can also be set using [the `TITAN_TEAM` system variable](/repo/docs/reference/system-environment-variables). If both are present, the flag value will override the system variable.
