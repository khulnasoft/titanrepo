...
Many monorepos can declare a `titan.json` in the root directory with a
...
To accommodate this, Titanrepo enables you to extend the root configuration with a `titan.json` in any package. This flexibility enables a more diverse set of apps and packages to co-exist in a Workspace, and allows package owners to maintain specialized tasks and configuration without affecting other apps and packages of the monorepo.

## How it works

To override the configuration for any task defined in the root `titan.json`, add
a `titan.json` file in any package of your monorepo with a top-level `extends`
key:

```jsonc title="./apps/my-app/titan.json"
{
  "extends": ["//"],
  "tasks": {
    "build": {
...
respective `package.json`. You _could_ configure Titanrepo to run these tasks
with a single `titan.json` at the root like this:
...
configuration in the SvelteKit package in `apps/my-svelte-kit-app/titan.json`:

```jsonc title="./apps/my-svelte-kit-app/titan.json"
{
  "extends": ["//"],
  "tasks": {
    "build": {
...
This means that your root `titan.json` has to have an empty `compile` task
entry:
...
`apps/my-custom-app/titan.json`,

```json title="./apps/my-app/titan.json"
{
  "extends": ["//"],
  "tasks": {
    "build": {
...
[`package#task` syntax](/repo/docs/crafting-your-repository/configuring-tasks#depending-on-a-specific-task-in-a-specific-package) in the root `titan.json`. The features are
similar, but have one significant difference: when you declare a package-specific task
in the root `titan.json`, it _completely_ overwrites the baseline task
...
Although the general idea is the same as the root `titan.json`, Package
...
```jsonc title="./apps/my-nextjs-app/titan.json"
{
  "tasks": {
    "my-nextjs-app#build": {
...
### Root titan.json cannot use the `extends` key

To avoid creating circular dependencies on packages, the root `titan.json`
cannot extend from anything. The `extends` key will be ignored.

## Troubleshooting

In large monorepos, it can sometimes be difficult to understand how Titanrepo is
interpreting your configuration. To help, we've added a `resolvedTaskDefinition`
to the [Dry Run](/repo/docs/reference/run#--dry----dry-run) output. If you run `titan run build --dry-run`, for example, the
output will include the combination of all `titan.json` configurations that were
