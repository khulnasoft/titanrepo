---
title: ls
description: API reference for the `titan ls` command
---
...
titan ls web @repo/ui [package(s)]
```
...
- You can override `main` as the default base by setting `TITAN_SCM_BASE`.
- You can override `HEAD` as the default head by setting `TITAN_SCM_HEAD`.

```bash title="Terminal"
TITAN_SCM_BASE=development titan ls --affected
```
