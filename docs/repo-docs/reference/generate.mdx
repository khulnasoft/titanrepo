---
title: generate
description: API reference for the `titan generate` command
---

import { Callout } from '#/components/callout';

Extend your Titanrepo with new apps and packages.

```bash title="Terminal"
titan generate
```

- [`titan generate run [generator-name]`](#run-generator-name): Run custom generators defined in your repository.
- [`titan generate workspace [options]`](#workspace): Create a new package in your repository by copying an existing one or from the start.
...
<Callout type="good-to-know">
  `titan gen` is an alias for `titan generate`. Additionally, `run` is the
  default command so `titan gen` is equivalent to `titan generate run`.
</Callout>
...
Default: `titan/generators/config.js`

#### `--root <path>`

The root of your repository

Default: directory with root `titan.json`
...
The root of your repository. Defaults to the directory of the root `titan.json`.
