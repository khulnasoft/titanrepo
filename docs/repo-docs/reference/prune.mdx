---
title: prune
description: API reference for the `titan prune` command
---
...
Run `titan prune frontend` to generate a pruned workspace for the `frontend` application in an `out` directory:
...
Using the same example from above, running `titan prune frontend --docker` will generate the following:
