...
description: Titanrepo builds a Task Graph based on your configuration and repository structure.
---
...
The Package Graph is the structure of your monorepo created by your package manager. When you install [Internal Packages](/repo/docs/core-concepts/internal-packages) into each other, Titanrepo will automatically identify those dependency relationships to build a foundational understanding of your Workspace.

This sets the groundwork for the Task Graph, where you'll define how **tasks** relate to each other.

## Task Graph

In `titan.json`, you express how tasks relate to each other. You can think of these relationships as
dependencies between tasks, but we have a more formal name for them: the Task
Graph.

Titanrepo uses a data structure called a [directed acyclic graph (DAG)](https://en.wikipedia.org/wiki/Directed_acyclic_graph) to
...
Titanrepo will build a task graph like this:
...
the `ui` package does not. You also have a `titan.json` that configures the
`build` task the same way as above with `"dependsOn": ["^build"]`. When you run
`titan run build`, what would you expect to happen?

Titanrepo will build this Task Graph:
...
Titanrepo calls the `ui` package a Transit Node in this scenario, because it
doesn't have its own `build` script. Since it doesn't have a `build` script,
Titanrepo won't execute anything for it, but it's still part of the graph for
...
Titanrepo's mental model is that all nodes in the Task Graph are the same. In other words,
...
`test` tasks, but only one app has a `build` task. Titanrepo's mental model
says that when you run `titan run build`, even if an app doesn't implement `build`
