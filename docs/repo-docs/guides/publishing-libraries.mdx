...
That way, when `tsup` is run the outputs can be [cached](/repo/docs/crafting-your-repository/caching) by Titanrepo.
...
Before we can run `titan run build`, there's one thing we need to consider. We've just added a [task dependency](/repo/docs/crafting-your-repository/running-tasks) into our monorepo. The `build` of `packages/math` needs to run **before** the `build` of `apps/web`.
...
If we've already set up [dev scripts](/repo/docs/crafting-your-repository/developing-applications#configuring-development-tasks) in our `titan.json`, running `titan run dev` will run our `packages/math` dev task in parallel with our `apps/web` dev task.
...
Linking your publishing flow into Titanrepo can make organizing your deploy a lot simpler and faster.
