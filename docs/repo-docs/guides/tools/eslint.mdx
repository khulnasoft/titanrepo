...
module.exports = {
  extends: [
    ...
    // Titanrepo custom eslint configuration configures the following rules:
    //  - https://github.com/khulnasoft/titanrepo/blob/main/packages/eslint-plugin-titan/docs/rules/no-undeclared-env-vars.md
    'eslint-config-titan',
  ].map(require.resolve),
  parserOptions: {
    project,
...
It's a typical ESLint config that extends the [Vercel style guide](https://github.com/khulnasoft/style-guide), nothing fancy.
...
This setup ships by default when you [create a new monorepo](/repo/docs/crafting-your-repository#from-zero-to-titan) with `npx create-titan@latest`. You can also look at [our basic example](https://github.com/khulnasoft/titanrepo/tree/main/examples/basic) to see a working version.
...
With your scripts prepared, you can then create your Titanrepo task:

```bash title="./titan.json"
{
  "tasks": {
    "lint": {}
  }
...
You can now run `titan lint` with [global `titan`](/repo/docs/getting-started/installation#global-installation) or create a script in your root `package.json`:
