---
title: Prisma
description: Learn how to use Prisma in a Titanrepo.
---
...
Let's also add these scripts to `titan.json` in the root:
...
Now, run `titan db:push db:generate` from the root of our repository to automatically migrate our database and generate our type-safe Prisma client.
...
<Callout type="info">
  ...
  when using Titanrepo because it automatically parallelizes the tasks.
</Callout>
...
You now have a reusable `@repo/db` package that you can import into any of your applications and a `titan db:push` script to push schema changes

However, your `db:generate` scripts aren't optimized yet. They provide crucial code to our `dev` and `build` tasks. If a new developer runs `titan dev` on an application without running `db:generate` first, they'll get errors.
...
`prisma generate` outputs files to the filesystem, usually inside `node_modules`. In theory, it should be possible to cache the output of `prisma generate` with Titanrepo to save a few seconds.
