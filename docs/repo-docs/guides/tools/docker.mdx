...
Docker is pretty smart about how it deploys your apps. Just like Titanrepo, it tries to do as [little work as possible](https://bitjudo.com/blog/2014/03/13/building-efficient-dockerfiles-node-dot-js/).
...
The solution is to prune the inputs to the Dockerfile to only what is strictly necessary. Titanrepo provides a simple solution - `titan prune`.

```bash title="Terminal"
titan prune api --docker
```
...
By default, `titan prune` puts all relevant files inside `./out`. But to optimize caching with Docker, we ideally want to copy the files over in two stages.
...
Our detailed [`with-docker` example](https://github.com/khulnasoft/titanrepo/tree/main/examples/with-docker) goes into depth on how to use `prune` to its full potential. Here's the Dockerfile, copied over for convenience.
...
# RUN yarn global add titan@^2
RUN yarn global add titan@^<your-major-version>
COPY . .

# Generate a partial monorepo with a pruned lockfile for a target workspace.
# Assuming "web" is the name entered in the project's package.json: { name: "web" }
RUN titan prune web --docker
...
RUN yarn titan run build --filter=web...
...
Assuming you are using a Dockerfile similar to the one above, we will bring in some environment variables from build arguments right before `titan build`:

```docker title="./apps/api/Dockerfile"
ARG TITAN_TEAM
ENV TITAN_TEAM=$TITAN_TEAM

ARG TITAN_TOKEN
ENV TITAN_TOKEN=$TITAN_TOKEN

RUN yarn titan run build --filter=web...
```

`titan` will now be able to hit your Remote Cache. To see a Titanrepo cache hit for a non-cached Docker build image, run a command like this one from your project root:

```bash title="Terminal"
docker build -f apps/web/Dockerfile . --build-arg TITAN_TEAM=“your-team-name” --build-arg TITAN_TOKEN=“your-token“ --no-cache
```
