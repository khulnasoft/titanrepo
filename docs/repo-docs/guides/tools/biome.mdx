---
title: Biome
description: Learn how to use Biome in your Titanrepo projects.
---
...
## Using Biome with Titanrepo

Biome is a rare exception to most tools that are used with Titanrepo because it is **so extraordinarily fast**. For this reason, we recommend using a [Root Task](/repo/docs/crafting-your-repository/configuring-tasks#registering-root-tasks) rather than creating separate scripts in each of your packages.
...
To create a Root Task, register the scripts to Titanrepo:
...
You'll now be able to run these scripts using `titan run format-and-lint` and `titan run format-and-lint:fix`.
