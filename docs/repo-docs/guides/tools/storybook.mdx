---
title: Storybook
description: Learn how to use Storybook in a Titanrepo.
---
...
[Storybook](https://storybook.js.org/) is a popular way to build UI components in an isolated environment. By putting Storybook into your Titanrepo, you can easily develop your design system right alongside your applications.

## Quickstart

If you'd rather use a template, this guide is walking through how to build [this Storybook/Titanrepo template](https://khulnasoft.com/templates/react/titanrepo-design-system) on Vercel.

<PackageManagerTabs>
<Tab>

```bash title="Terminal"
npx create-titan@latest -e design-system
```
...
```bash title="Terminal"
yarn dlx create-titan@latest -e design-system
```
...
```bash title="Terminal"
pnpm dlx create-titan@latest -e design-system
```
...
If you don't have an existing project, use [create-titan](/repo/docs/getting-started/installation) to create a new monorepo:
...
Last, integrate the new Storybook application into your Titanrepo:
...
These scripts will now run with the `titan dev` and `titan build` tasks in your `titan.json`.

To ensure file outputs are cached when you run `build`, add `storybook-static` to the outputs of your `titan.json` build task:
...
Run `titan build` to build the Storybook application alongside the rest of your applications.

You can also run `titan build` again to see cache hits for your builds.
...
To prevent this, exclude stories from the inputs to your `build` task in your root `titan.json`. You'll also need to create a `build:storybook` task, which you'll need in a moment:
...
```json title="./titan.json"
{
  "tasks": {
    "build": {
      "dependsOn": ["^build"],
      "inputs": ["$TITAN_DEFAULT$", "!**/*.stories.{tsx,jsx,mdx}"], // [!code highlight]
...
```json title="./apps/storybook/titan.json"
{
  "extends": ["//"],
  "tasks": {
    "build:storybook": {
...
1. Run `titan build:storybook build`. You should see cache misses.
2. Run `titan build:storybook build` again. You should see all cache hits.
3. Make a code change **to a story** in your `@repo/ui` package.
4. Run `titan build:storybook build` again. You should **only** see a cache miss for the Storybook application. All others should hit cache.
