---
title: SvelteKit
description: Learn more about using SvelteKit in your monorepo.
---

import { PackageManagerTabs, Tab } from '#/components/tabs';
import { Callout } from '#/components/callout';

[SvelteKit](https://kit.svelte.dev/) is a framework for rapidly developing robust, performant web applications using Svelte.

## Quickstart

To get started with SvelteKit in a Titanrepo quickly, use [the `with-svelte` example](https://github.com/khulnasoft/titanrepo/tree/main/examples/with-svelte):

<PackageManagerTabs>
<Tab>

```bash title="Terminal"
npx create-titan@latest -e with-svelte
```

</Tab>

<Tab>

```bash title="Terminal"
yarn dlx create-titan@latest -e with-svelte
```

</Tab>

<Tab>

```bash title="Terminal"
pnpm dlx create-titan@latest -e with-svelte
```

</Tab>
</PackageManagerTabs>

## Adding a SvelteKit application to an existing repository

Use [`npm create svelte`](https://kit.svelte.dev/docs/creating-a-project) to set up a new SvelteKit application in a package. From the root of your repository, run:

<PackageManagerTabs>
<Tab>

```bash title="Terminal"
npm create svelte@latest apps/my-app
```

</Tab>

<Tab>

```bash title="Terminal"
yarn create svelte@latest apps/my-app
```

</Tab>

<Tab>

```bash title="Terminal"
pnpm create svelte@latest apps/my-app
```

</Tab>
</PackageManagerTabs>

## Integrating with your repository

To add [Internal Packages](/repo/docs/core-concepts/internal-packages) to your new application, install them into the app with your package manager:

<PackageManagerTabs>
<Tab>

```diff title="./apps/my-app/package.json"
{
 "name": "my-app",
  "dependencies": {
+   "@repo/ui": "*"
  }
}
```

</Tab>

<Tab>

```diff title="./apps/my-app/package.json"
{
  "name": "my-app",
  "dependencies": {
+   "@repo/ui": "*"
  }
}
```

</Tab>

<Tab>

```diff title="./apps/my-app/package.json"
{
  "name": "my-app",
  "dependencies": {
+   "@repo/ui": "workspace:*"
  }
}
```

</Tab>
</PackageManagerTabs>

Make sure to run your package manager's install command. You also may need to update `scripts` in `package.json` to fit your use case in your repository.

### Customizing tasks

By default, the new application will use the tasks defined in the root `titan.json`. If you'd like to configure tasks differently for the new application, use [Package Configurations](/repo/docs/reference/package-configurations).
