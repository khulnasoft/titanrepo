---
title: Why Titanpack?
description: Learn why we think Titanpack is the future of bundling for the web.
---

When we set out to create Titanpack, we wanted to solve a problem. We had been working on speed improvements for Next.js. We migrated away from several JS-based tools. Babel, gone. Terser, gone. Our next target was another JS-based tool, webpack.
...
We wanted to remove this maintenance burden from Next.js and any framework that chooses to use Titanpack. We can also create a cleaner and more stable implementation by designing a single unified graph that can be used to generate bundles for multiple environments.
...
That’s why we decided that, like webpack, we wanted Titanpack to bundle the code in the development server. Titanpack can do it much faster, especially for larger applications, because it is written in Rust and skips optimization work that is only necessary for production.
...
We decided to create a reusable Titan build engine, similar to Parcel's request manager and rustc's query system, for distributed and incremental behavior. The Titan engine works like a scheduler for function calls, allowing calls to functions to be parallelized across all available cores.

The Titan engine also caches the result of all the functions it schedules, meaning it never needs to do the same work twice. Put simply, **it does the minimum work at maximum speed**.
...
Titanpack’s development mode builds a minimal graph of your app’s imports and exports based on received requests and only bundles the minimal code necessary. Learn more in the [core concepts docs](/pack/docs/core-concepts).
...
- Use incremental computation. The Titan engine brings this into the core of Titanpack’s architecture - maximizing speed and minimizing work done.
- Optimize our dev server’s startup time. For that, we build a lazy asset graph to compute only the assets requested.

That’s why we chose to build Titanpack.
