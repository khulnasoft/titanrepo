---
title: TypeScript
description: Learn more about TypeScript in Titanpack.
---

Titanpack supports [TypeScript](https://www.typescriptlang.org/) out of the box. This means you can import `.ts` files with Titanpack. We support all of TypeScript's feature set.
...
Titanpack reads the `paths` and `baseUrl` in `tsconfig.json` in order to resolve these paths, just like `Next.js` does.

This means you only need to configure your absolute paths in one place.

## Type Checking

Titanpack does not perform type checks on your application. We use [SWC](https://swc.rs/) to compile TypeScript code, which also does not perform type checks.
