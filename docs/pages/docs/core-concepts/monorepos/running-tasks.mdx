---
title: Running Tasks
description: Titanrepo helps you specify task dependencies declaratively.
---
...
## Titanrepo can multitask

Titanrepo can schedule our tasks for maximum speed by understanding the dependencies between our tasks.

First, we declare our tasks inside `titan.json`:
...
- yarn workspaces run test
- yarn workspaces run build
+ titan run lint test build
```

When we run it, Titanrepo will **multitask as many tasks as possible over all available CPU's**, meaning our tasks run like this:

![](../../../../public/images/docs/titanrepo-excalidraw.png)

Both `lint` and `test` run immediately, because they have no `dependsOn` specified in `titan.json`.
...
Let's walk through some common patterns you'll want to get to know before diving in to `titan.json`.
...
This means that whenever `titan run deploy` is run, `build` will also be run inside the same workspace.
...
1. Although this `<workspace>#<task>` syntax is a useful escape hatch, we generally recommend using it for deployment orchestration tasks such as health checks, rather than build-time dependencies, so that Titanrepo can optimize these tasks more efficiently
...
`titan` can run tasks that exist in the `package.json` file at the root of the monorepo.
...
`titan run build`, you must opt the root into it by declaring `"//#build": {...}` in your configuration.
...
```jsonc
{
  ...
  "pipeline": {
    ...
    // "titan run test" is run
    "//#test": {
      "dependsOn": [],
      "outputs": []
    },
    // This will cause the "format" script in the root package.json
    // to be run when "titan run format" is run. Since the general
...
**A note on recursion**: Scripts defined in the monorepo's root `package.json` often call `titan` themselves.
For example, the `build` script might be `titan run build`. In this situation, including `//#build` in
`titan run build` will cause infinite recursion. It is for this reason that tasks run from the monorepo's root must
be explicitly opted into via including `//#<task>` in the pipeline configuration. `titan` includes
some best-effort checking to produce an error in the recursion situations, but it is up to you to to only
opt in those tasks which don't themselves trigger a `titan` run that would recurse.

<Callout type="idea" icon={<HeartIcon  className="h-5 w-5 mt-1 text-gray-400" aria-hidden="true" />}>
Titanrepo's Pipeline API design and this page of documentation was inspired by [Microsoft's Lage project](https://microsoft.github.io/lage/docs/Tutorial/pipeline#defining-a-pipeline).
...
Sometimes tasks declared in the `pipeline` are not present in all workspaces' `package.json` files. `titan` will gracefully ignore those. No problem!

#### `pipeline` tasks are the only ones that `titan` knows about

`titan` will only account for tasks declared in the `pipeline` configuration. If it's not listed there, `titan` will not know how to run them.
