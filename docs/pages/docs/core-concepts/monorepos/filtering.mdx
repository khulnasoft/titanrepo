...
A monorepo can contain hundreds, or thousands, of workspaces. By default, running `titan run test` will execute the `test` task in **all available workspaces**.

![Without using a filter, test will be run across all packages](../../../../public/images/docs/no-filter.png)

Titanrepo supports a `--filter` flag that lets you **select the workspaces you'd like to execute your task in**.
...
Titanrepo will run each task against each matched workspace, ensuring that any tasks which depend on it are run first, according to the `pipeline` specification in [`titan.json`](/docs/reference/configuration#pipeline).
...
```sh
titan run build --filter=my-pkg --filter=my-app
```
...
```sh
# Build 'my-pkg', letting `titan` infer task dependencies
# from the pipeline defined in titan.json
titan run build --filter=my-pkg

# Build '@acme/bar', letting `titan` infer task dependencies
# from the pipeline defined in titan.json
titan run build --filter=@acme/bar
```

If you want to run tasks inside several workspaces with similar names, you can use glob syntax: `--filter=*my-pkg*`.

```sh
# Build all workspaces that start with 'admin-', letting titan infer task
# dependencies from the pipeline defined in titan.json
titan run build --filter=admin-*
```
...
```diff
- titan run build --filter=@acme/ui
+ titan run build --filter=ui
```
...
titan run test --filter=[main...my-feature]
```
...
titan run build --filter=@foo/bar...[HEAD^1]
```
...
# changed in the last commit
titan run test --filter=@scope/*{./packages/*}[HEAD^1]
```
