---
title: Caching
description: Caching allows Titanrepo to skip work that's already been done, for the fastest builds.
---
...
Every JavaScript or TypeScript codebase will need to run `package.json` scripts, like `build`, `test` and `lint`. In Titanrepo, we call these **tasks**.

Titanrepo can cache the results and logs of your tasks - leading to enormous speedups for slow tasks.
...
Let's say you run a `build` task with Titanrepo using `titan run build`:

![](../../../public/images/docs/cache-miss.png)

1. Titanrepo will **evaluate the inputs to your task** (by default all non-gitignored files in the workspace folder) and **turn them into a hash** (e.g. `78awdk123`).

2. **Check the local filesystem cache** for a folder named with the hash (e.g.`./node_modules/.cache/titan/78awdk123`).

3. If Titanrepo doesn't find any matching artifacts for the calculated hash, Titanrepo will then **execute the task**.

4. Once the task is over, Titanrepo **saves all the outputs** (including files and logs) into its cache under the hash.

<Callout type="info">
  Titanrepo takes a lot of information into account when creating the hash - source files, environment variables, and even the source files of dependent workspaces. Learn more [below](/docs/core-concepts/caching#hashing).
</Callout>
...
2. Titanrepo will find the folder in its cache with the calculated hash (e.g. `./node_modules/.cache/titan/78awdk123`)

3. **Instead of running the task**, Titanrepo will **replay the output** - printing the saved logs to `stdout` and restoring the saved output files to their respective position in the filesystem.

Restoring files and logs from the cache happens near-instantaneously. This can take your build times from minutes or hours down to seconds or milliseconds. Although specific results will vary depending on the shape and granularity of your codebase's dependency graph, most teams find that they can cut their overall monthly build time by around 40-85% with Titanrepo's caching.

## Configuring Cache Outputs

Using [`pipeline`](../reference/configuration#pipeline), you can configure cache conventions across your Titanrepo.
...
If your task does not emit any files (e.g. unit tests with Jest), set `outputs` to an empty array (i.e. `[]`), and Titanrepo will cache the logs for you.

When you run `titan run build test`, Titanrepo will execute your build and test scripts,
and cache their `output`s in `./node_modules/.cache/titan`.
...
titan run dev --parallel --no-cache
```
...
of files `titan` considers for a particular task. In this case, only consider `.ts` and `.tsx` files relevant for
...
<Callout type="info">
  `titan.json` is *always* considered a global dependency. If you modify `titan.json`, all caches
  are invalidated.
</Callout>

## Altering Caching Based on Environment Variables

When you use `titan` with tools that inline environment variables at build time
(e.g. [Next.js][1] or [Create React App][2]), it is important to tell `titan` about it.
Otherwise, you could ship a cached build with the wrong environment variables!

You can control `titan`'s caching behavior based on
...
To help ensure correct caching across environments, Titanrepo automatically infers and includes public environment variables when calculating cache keys for apps built with detected frameworks. You can safely omit framework-specific public environment variables from `titan.json`:
...
Note that this automatic detection and inclusion only works if Titanrepo successfully infers the framework your apps are built with. The supported frameworks and the environment variables that Titanrepo will detect and include in the cache keys:
...
<Callout type="info">
  There are some exceptions to the list above. For various reasons, CI systems (including Khulnasoft)
  set environment variables that start with these prefixes even though they aren't part of your build
  output. These can change unpredicably &mdash; even on every build! &mdash; invalidating Titanrepo's
  cache. To workaround this, Titanrepo uses a `TITAN_CI_VENDOR_ENV_KEY` variable to
  _exclude_ environment variables from Titanrepo's inference.

For example, Khulnasoft sets the `NEXT_PUBLIC_KHULNASOFT_GIT_COMMIT_SHA`. This value changes on every build,
so Khulnasoft _also_ sets `TITAN_CI_VENDOR_ENV_KEY="NEXT_PUBLIC_KHULNASOFT_"` to exclude these variables.

Luckily, you only need to be aware of this on other build systems; you don't need to worry
about these edge cases when using Titanrepo on Khulnasoft.
...
To further assist in detecting unseen dependencies creeping into your builds, and to help ensure that your Titanrepo cache is correctly shared across every environment, use the [`eslint-config-titan`](https://www.npmjs.com/package/eslint-config-titan) package. While automatic environment variable inclusion should cover most situations with most frameworks, this ESLint config will provide just-in-time feedback for teams using other build time inlined environment variables. This will also help support teams using in-house frameworks that we cannot detect automatically.
...
For more control over the rules, you can install and configure the [`eslint-plugin-titan`](https://www.npmjs.com/package/eslint-plugin-titan) _plugin_ directly by first adding it to plugins and then configuring the desired rules:
...
Since Titanrepo runs _before_ your tasks, it is possible for your tasks to create or mutate environment variables after `titan` has already calculated the hash for a particular task. For example, consider this `package.json`:
...
`titan`, having calculated a task hash prior to invoking the `build` script, will be unable to discover the `NEXT_PUBLIC_GA_ID=UA-00000000-0` environment variable and thus unable to partition the cache based upon that, or any environment variable configured by `dotenv`.

Be careful to ensure that all of your environment variables are configured prior to invoking `titan`!

## Force overwrite cache

Conversely, if you want to force `titan` to re-execute a previously cached task, add the `--force` flag:
...
Not only does `titan` cache the output of your tasks, it also records the terminal output (i.e. combined `stdout` and `stderr`) to (`<package>/.titan/run-<command>.log`). When `titan` encounters a cached task, it will replay the output as if it happened again, but instantly, with the package name slightly dimmed.

## Hashing

By now, you're probably wondering how `titan` decides what constitutes a cache hit vs. miss for a given task. Good question!

First, `titan` constructs a hash of the current global state of the codebase:
...
Once `titan` encounters a given workspace's task in its execution, it checks the cache (both locally and remotely) for a matching hash. If it's a match, it skips executing that task, moves or downloads the cached output into place and replays the previously recorded logs instantly. If there isn't anything in the cache (either locally or remotely) that matches the calculated hash, `titan` will execute the task locally and then cache the specified `outputs` using the hash as an index.

The hash of a given task is injected at execution time as an environment variable `TITAN_HASH`. This value can be useful in stamping outputs or tagging Dockerfile etc.

<Callout>
  As of `titan` v0.6.10, `titan`'s hashing algorithm when using `npm` or `pnpm`
  differs slightly from the above. When using either of these package managers,
  `titan` will include the hashed contents of the lockfile in its hash
