...
## Setup with Titanrepo

You should specify your `dev` task like this in your `titan.json`.
...
{
  "scripts": {
    "dev": "dotenv -- titan run dev"
  }
}
```

This will extract the environment variables from `.env` before running `titan run dev`.
...
<Callout>
  You should also [add your environment variables](/docs/core-concepts/caching#altering-caching-based-on-environment-variables) to your `titan.json` if you're using them to build your app.
</Callout>
