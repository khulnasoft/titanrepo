...
description: This guide aims to help you debug issues with your Titanrepo builds and configuration.
---

# Troubleshooting

This guide aims to help you debug issues with your Titanrepo builds and configuration.
...
- Are you properly bundling and transpiling the dependency before building the application?
  ...
  - If you are using Next.js, you might be using `next-transpile-modules`. Ensure you add the name of the dependency inside `next.config.js` ([example](https://github.com/khulnasoft/titanrepo/blob/main/examples/basic/apps/docs/next.config.js#L1)).
...
- Is any source code being generated during the build that isn't checked into git?
  - This would change the fingerprint Titanrepo uses to store build outputs.
- Are [cache outputs correctly specified](./core-concepts/caching#configuring-cache-outputs) in your Titanrepo [pipeline](./core-concepts/pipelines#defining-a-pipeline)?
...
- [Are relevant inlined environment variables accounted for?](./core-concepts/caching#alter-caching-based-on-environment-variables-and-files)
  - To verify, run `titan` in verbose mode by adding `-vvv` to `titan run <task>` and look at which environment variables are included in the hashes.

## I'm seeing cache hits, but my build is broken

- Are [cache outputs properly specified](./core-concepts/caching#configuring-cache-outputs) in your Titanrepo [pipeline](./core-concepts/pipelines#defining-a-pipeline)?
  - Pipeline settings are not inherited or merged, so they need to be re-specified in [workspace-specific tasks](./core-concepts/pipelines#implicit-dependencies-and-specific-package-tasks) (e.g. `web#build` does **not** inherit pipeline settings from `build`).
