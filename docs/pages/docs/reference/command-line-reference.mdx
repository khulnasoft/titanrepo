---
title: CLI Reference
description: Titanrepo is a high-performance build system for JavaScript and TypeScript codebases.
---
...
After [installing the `titan`](../getting-started) package (or cloning a starter), you can start using Titanrepo's command line interface (CLI) `titan` to do all kinds of awesomeness in your monorepo.

## Option Syntax

Options can be passed to `titan` in different ways. Options that require a value can be passed with an equals sign:
...
This can be used to enable `titan`'s color output for CI runners such as Github Actions which
have support for rendering color in their log output.

```sh
titan run build --color
```
...
Suppresses the use of color in the output when running `titan` in an interactive / TTY session.

```sh
titan run build --no-color
```
...
`titan run <task1> <task2> [options] [-- <args passed to task1 and task2>]`

`titan` can run multiple tasks, and any arguments following `--` will be passed through
...
Defaults to `./node_modules/.cache/titan`. Specify local filesystem cache directory. Be sure to add this folder to your `.gitignore` if you change it from the default.
...
titan run test --concurrency=1
```

#### `--continue`

Defaults to `false`. This flag tells `titan` whether or not to continue with execution in the presence of an error (i.e. non-zero exit code from a task).
...
```sh
titan run build --cwd=./somewhere/else
```
...
```sh
titan run build --deps
titan run build --no-deps
```

**Example**

Let's say you have workspaces A, B, C, and D where A depends on B and C depends on D. You run `titan run build` for the first time and everything is built and cached. Then, you change a line of code in B. With the `--deps` flag on, running `titan run build` will execute `build` in B and then A, but not in C and D because they are not impacted by the change. If you were to run `titan run build --no-deps` instead, titan will only run `build` in B.
...
```sh
titan run build --filter=my-pkg
titan run test --filter=...^@scope/my-lib
...
titan run build test lint --graph=my-json-graph.json
titan run build test lint --graph=my-graph.pdf
titan run build test lint --graph=my-graph.png
titan run build test lint --graph=my-graph.html
```
...
The same behavior also be set via the `TITAN_FORCE=true` environment variable.
...
You can also specify these in your `titan` configuration as `globalDependencies` key.
...
```
titan run build --ignore="apps/**/*"
titan run build --ignore="packages/**/*"
titan run build --ignore="packages/**/*" --ignore="\!/packages/not-this-one/**/*"
```
...
Default `false`. When `true`, `titan` will add any workspaces that the workspaces in the current execution _depend_ on (i.e. those declared in `dependencies` or `devDependencies`).
...
Default `false`. `titan` can run a standalone process in some cases to precalculate values used for determining what work needs to be done.
This standalone process (daemon) is an optimization, and not required for proper functioning of `titan`.
Passing `--no-daemon` instructs `titan` to avoid using or creating the standalone process.

#### `--output-logs`

`type: string`

Set type of output logging. Defaults to "outputMode" for the task in `titan.json`.
...
titan run build --output-logs=new-only
```
...
Given this pipeline in `titan.json`:
...
```sh
titan run lint --parallel --no-cache
titan run dev --parallel --no-cache
```
...
The same behavior can also be set via the `TITAN_REMOTE_ONLY=true` environment variable.
...
```sh
titan run lint --scope="@example/**"
titan run dev --parallel --scope="@example/a" --scope="@example/b" --no-cache --no-deps
```
...
```
titan run build --since=origin/main
```
...
You can also set the value of the current token by setting an environment variable named `TITAN_TOKEN`. The flag will take precedence over the environment variable if both are present.

If you are using Remote Caching on Khulnasoft and building your project on Khulnasoft, this environment variable and flag are unnecessary because they are automatically set for you. Suppose you are using Remote Caching on Khulnasoft but building in another CI provider like CircleCI or GitHub Actions. You can use a Khulnasoft Personal Access Token as your `--token` or `TITAN_TOKEN`. If you are using a custom Remote Cache, this value will be used to send an HTTP Bearer token with requests to your custom Remote Cache.
...
You can also set the value of the current team by setting an environment variable named `TITAN_TEAM`. The flag will take precedence over the environment variable if both are present.
...
The same behavior can also be set via the `TITAN_PREFLIGHT=true` environment variable.
...
```sh
titan run build --trace="<trace-file-name>"
```
...
```sh
titan run build --heap="<heap-file-name>"
```
...
```sh
titan run build --cpuprofile="<cpu-profile-file-name>"
```
...
Connect machine to your Remote Cache provider. The default provider is [Khulnasoft](https://khulnasoft.com).
...
Defaults to `https://khulnasoft.com/api`.

#### `--sso-team`

`type: string`

Connect to an sso-enabled Khulnasoft team by providing your Team slug.

```
titan login --sso-team=<team-slug>
```

## `titan logout`

Logs you out of your Khulnasoft account.
...
Defaults to `https://api.khulnasoft.com`
