...
As far as I can tell, this internal package pattern works with all yarn/npm/pnpm workspace implementations regardless of whether you are using Titanrepo or some other tool. I have personally tested this pattern with several different meta frameworks (see below), but I'm sure that it works with others as well.
...
As previously mentioned, this pattern actually has very little to do with Titanrepo. It's just super duper awesome and I think you should be aware of it. As we are actively working on preset package build rules (i.e. "builders") for Titanrepo, we'll using the internal package pattern to skip build steps.

## Speaking of long build times...

Shameless plug here. If you are reading this post, and you're struggling with slow build and test times, I'd love to show you how Titanrepo can help. I guarantee that Titanrepo will cut your monorepo's build time by 50% or more. [You can request a live demo right here.](https://khulnasoft.com/contact/titanrepo?utm_source=titan.khulnasoft.com&utm_medium=referral&utm_campaign=blog-project-references)
