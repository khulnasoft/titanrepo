...
import type { Schema, Pipeline } from "titan-types";
...
export default function migrateEnvVarDependencies(
  ...
  if (files.length === 1) {
    ...
    if (!fs.existsSync(titanConfigPath)) {
      error(`No titan.json found at ${root}. Is the path correct?`);
      process.exit(1);
    }
    ...
    if (hasLegacyEnvVarDependencies(rootTitanJson).hasKeys) {
      ...
      ok(
        'no unmigrated environment variable dependencies found in "titan.json"'
      );
      process.exit(0);
    }
