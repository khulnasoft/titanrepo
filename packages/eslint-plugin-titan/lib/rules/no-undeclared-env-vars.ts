...
const meta: Rule.RuleMetaData = {
  ...
  docs: {
    description:
      "Do not allow the use of `process.env` without including the env key in titan.json",
    category: "Configuration Issues",
    recommended: true,
    url: `https://github.com/khulnasoft/titanrepo/tree/main/packages/eslint-plugin-titan/docs/rules/${RULES.noUndeclaredEnvVars}.md`,
  },
  schema: [
    {
      type: "object",
...
function create(context: Rule.RuleContext): Rule.RuleListener {
  ...
  const titanConfig = options?.[0]?.titanConfig;
  const titanVars = getEnvVarDependencies({
    cwd: context.getCwd(),
    titanConfig,
  });

  // if this returns null, something went wrong reading from the titan config
  ...
  const checkKey = (node: Node, envKey?: string) => {
    if (
      envKey &&
      !titanVars.has(envKey) &&
      !regexAllowList.some((regex) => regex.test(envKey))
    ) {
      context.report({
        node,
        message: "${{ envKey }} is not listed as a dependency in titan.json",
        data: { envKey },
      });
    }
  };
