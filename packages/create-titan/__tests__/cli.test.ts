  ...
  describe("--no-install", () => {
    it(
      ...
      async () => {
        ...
        expect(stdout).toContain(
          `? Where would you like to create your titanrepo? (./my-titanrepo)`
        );
        ...
        expect(stdout).toContain(
          ">>> Created a new titanrepo with the following:"
        );

        expect(stdout).toContain(
          `>>> Success! Created a new Titanrepo at "${testDir}"`
        );
...
async function runInteractiveCLI(
  ...
  return new Promise((resolve, reject) => {
    cli.stdout.on("data", (data) => {
      ...
      if (
        !advancedPath &&
        accumulator.match(/Where would you like to create your titanrepo/g)
      ) {
        advancedPath = true;
...
const testDirRegex = /^my-.+-titanrepo$/;
