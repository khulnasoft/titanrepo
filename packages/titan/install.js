...
const TITAN_VERSION = require("./package.json").version;

const toPath = path.join(__dirname, "bin", "titan");
let isToPathJS = true;
...
function validateBinaryVersion(...command) {
  ...
  const stdout = child_process
    ...
    .toString()
    .trim();
  if (stdout !== TITAN_VERSION) {
    throw new Error(
      `Expected ${JSON.stringify(TITAN_VERSION)} but got ${JSON.stringify(
        stdout
      )}`
    );
  }
...
function installUsingNPM(pkg, subpath, binPath) {
  // Erase "npm_config_global" so that "npm install --global titan" works.
  ...
  // Create a temporary directory inside the "titan" package with an empty
  // "package.json" file. We'll use this to run "npm install" in.
  const titanLibDir = path.dirname(require.resolve("titan"));
  const installDir = path.join(titanLibDir, "npm-install");
  ...
  try {
    ...
    child_process.execSync(
      `npm install --loglevel=error --prefer-offline --no-audit --progress=false ${pkg}@${TITAN_VERSION}`,
...
function maybeOptimizePackage(binPath) {
  ...
  // This package contains a "bin/titan" JavaScript file that finds and runs
  // the appropriate binary executable. However, this means that running the
  // "titan" command runs another instance of "node" which is way slower than
  ...
  // because on Windows the binary executable must be called "titan.exe"
  // instead of "titan".
  //
  ...
  if (os.platform() !== "win32" && !isYarn()) {
    const tempPath = path.join(__dirname, "bin-titan");
    try {
...
async function downloadDirectlyFromNPM(pkg, subpath, binPath) {
  ...
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg}-${TITAN_VERSION}.tgz`;
  console.error(`[titan] Trying to download ${JSON.stringify(url)}`);
  try {
    fs.writeFileSync(
      binPath,
    ...
    console.error(
      `[titan] Failed to download ${JSON.stringify(url)}: ${
        (e && e.message) || e
      }`
    );
    throw e;
...
async function checkAndPreparePackage() {
  ...
  try {
    ...
    console.error(`[titan] Failed to find package "${pkg}" on the file system

This can happen if you use the "--no-optional" flag. The "optionalDependencies"
package.json feature is used by titan to install the correct binary executable
for your current platform. This install script will now attempt to work around
this. If that fails, you need to remove the "--no-optional" flag to use titan.
`);
    ...
    // to be helpful but it's not the supported way of installing titan.
    binPath = downloadedBinPath(pkg, subpath);
    try {
      console.error(`[titan] Trying to install package "${pkg}" using npm`);
      installUsingNPM(pkg, subpath, binPath);
    } catch (e2) {
      console.error(
        `[titan] Failed to install package "${pkg}" using npm: ${
          (e2 && e2.message) || e2
        }`
      );
